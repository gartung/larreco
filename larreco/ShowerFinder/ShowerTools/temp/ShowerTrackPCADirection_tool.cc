...
namespace ShowerRecoTools {
  ...
  void ShowerTrackPCADirection::configure(const fhicl::ParameterSet& pset)
  {
  ...
  int ShowerTrackPCADirection::CalculateProperty(const art::Ptr<recob::PFParticle>& pfparticle,
					    art::Event& Event,
					    reco::shower::ShowerPropertyHolder& ShowerPropHolder){
    ...
    if (!Event.getByLabel(fHitModuleLabel, hitHandle)){
        throw cet::exception("ShowerTrackPCAFinder") << "Could not configure the hit handle. Something is configured incorrectly. Stopping";
    }
    ...
    for (auto hit : trackHits){
        const art::Ptr<recob::SpacePoint> sp = fosph.at(hit.key());
    ...
    if(trackSpacePoints.size() < fMinPCAPoints){
      mf::LogError("Shower3DTrackFinder") << "Not enough spacepoints for PCA, returning "<< std::endl;
      return 1;
    } 
    ...
    if (fDebugEVD){
        ...
        fTRACSAlg.DebugEVD(pfparticle,Event,ShowerPropHolder);
    }
