...
namespace {
  ...
  struct AddMany {
    AddMany(art::Ptr<T> const& ptr,
            art::Assns<T, U>& assns) :
      ptr_{ptr},
      assns_{assns}
    {}

    void operator()(art::Ptr<U> const& b) const {
...
shower::EMShower::produce(art::Event& evt)
{
  ...
  auto clusterAssociations =
    std::make_unique<art::Assns<recob::Shower, recob::Cluster>>();
  auto hitShowerAssociations =
    std::make_unique<art::Assns<recob::Shower, recob::Hit>>();
  auto trackAssociations =
    std::make_unique<art::Assns<recob::Shower, recob::Track>>();
  auto spShowerAssociations =
    std::make_unique<art::Assns<recob::Shower, recob::SpacePoint>>();
  auto hitSpAssociations =
  ...
  if (!pfpHandle.isValid()) {
    ...
    fEMShowerAlg.AssociateClustersAndTracks(
      clusters, fmh, fmt, clusterToTracks, trackToClusters);

    // Make initial showers
    std::vector<std::vector<int>> initialShowers =
      fEMShowerAlg.FindShowers(trackToClusters);

    // Deal with views in which 2D reconstruction failed
    std::vector<int> clustersToIgnore;
    if (fFindBadPlanes)
      clustersToIgnore =
        fEMShowerAlg.CheckShowerPlanes(initialShowers, clusters, fmh);
    ...
    for (size_t ipfp = 0; ipfp < pfps.size(); ++ipfp) {
      ...
      if (fCNNEMModuleLabel != "" &&
          fUseCNNtoIDEMPFP) { // use CNN to identify EM pfparticle
        auto hitResults =
          anab::MVAReader<recob::Hit, 4>::create(evt, fCNNEMModuleLabel);
        if (!hitResults) {
        ...
        if (fmcp.isValid()) { // find clusters
          ...
          for (size_t iclu = 0; iclu < clus.size(); ++iclu) {
            std::vector<art::Ptr<recob::Hit>> ClusterHits =
              fmh.at(clus[iclu].key());
            pfphits.insert(
              pfphits.end(), ClusterHits.begin(), ClusterHits.end());
          }
          if (pfphits.size()) { // find hits
            auto vout = hitResults->getOutput(pfphits);
            double trk_like = -1,
                   trk_or_em = vout[trkLikeIdx] + vout[emLikeIdx];
            if (trk_or_em > 0) {
  ...
  for (auto const& newShower : newShowers) {
    ...
    if (showerNum != fShower and fShower != -1)
      continue;
    ...
    for (int const showerCluster : newShower) {
      ...
      // Hits
      std::vector<art::Ptr<recob::Hit>> const& showerClusterHits =
        fmh.at(cluster.key());
      if (fCNNEMModuleLabel != "" &&
          fUseCNNtoIDEMHit) { // use CNN to identify EM hits
        auto hitResults =
          anab::MVAReader<recob::Hit, 4>::create(evt, fCNNEMModuleLabel);
        if (!hitResults) {
          throw cet::exception("EMShower")
    ...
    if (!pfpHandle
           .isValid()) { // For non-pfparticles, get space points from tracks
      ...
      art::FindManyP<recob::SpacePoint> fmspp(
        showerHits, evt, fPFParticleModuleLabel);
      if (fmspp.isValid()) {
    ...
    if (!pfpHandle.isValid()) {
      ...
      fEMShowerAlg.FindInitialTrack(
        showerHitsMap, initialTrack, initialTrackHits, fPlane);
      ...
      if (fMakeSpacePoints)
        showerSpacePoints =
          fEMShowerAlg.MakeSpacePoints(showerHitsMap, hitAssns);
      else {
      ...
      recob::Shower shower =
        fEMShowerAlg.MakeShower(showerHits, initialTrack, initialTrackHits);
      ...
      if (vertices.size()) {
        ...
        for (auto const& sp :
             showerSpacePoints_p | ranges::view::transform(to_element)) {
          double const dist2 = cet::sum_of_squares(nuvtx.X() - sp.XYZ()[0],
                                                   nuvtx.Y() - sp.XYZ()[1],
                                                   nuvtx.Z() - sp.XYZ()[2]);
          if (dist2 < mindist2) {
            mindist2 = dist2;
        ...
        for (auto const& vtx : vertices) {
          auto const pos = vtx->position();
          double const dist2 = cet::sum_of_squares(pos.X() - shwvtx.X(), pos.Y() - shwvtx.Y(), pos.Z() - shwvtx.Z());
          if (dist2 < mindist2) {
            mindist2 = dist2;
            bestvtx = vtx;
          }
        ...
        int iok = 0;
        recob::Shower shower =
          fEMShowerAlg.MakeShower(showerHits, bestvtx, iok);
        if (iok == 0) {
          showers->push_back(shower);
