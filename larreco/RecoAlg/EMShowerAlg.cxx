  ...
  for (auto const& clusterPtr : clusters) {
    ...
    for (auto const& clusterHitPtr : clusterHits) {
      ...
      if (clusterHitTrackPtr->Length() < fMinTrackLength) {
        if (fDebug > 2)
          std::cout << "Track " << clusterHitTrackPtr->ID()
                    << " is too short! (" << clusterHitTrackPtr->Length()
                    << ")\n";
        continue;
      }
      ...
      if (cet::search_all(clustersToIgnore, cluster))
        continue;
  ...
  std::map<int, std::vector<art::Ptr<recob::Hit>>>& showerHitsMap) const
{
  ...
  for (std::vector<art::Ptr<recob::Hit>>::iterator hitIt =
         showerHitsMap.at(problemPlane).begin();
       hitIt != showerHitsMap.at(problemPlane).end() and
       (*hitIt)->WireID().TPC == wrongTPC;
       ++hitIt)
    ++nHits;

  // If there are more than two hits in the 'wrong TPC', we can't be sure it is
  // indeed wrong
  if (nHits > 2)
    return;
  ...
  for (std::vector<art::Ptr<recob::Hit>>::iterator hitIt =
       ...
       hitIt != showerHitsMap.at(problemPlane).end() and
       std::distance(std::next(showerHitsMap.at(problemPlane).begin(), nHits),
                     hitIt) < 4 * nHits;
       ++hitIt)
  ...
  // If we get this far, we can move the problem hits from the front of the
  // shower to the back
  std::map<int, int> tpcCount;
  for (std::vector<int>::iterator otherPlaneIt = otherPlanes.begin();
       otherPlaneIt != otherPlanes.end();
       ++otherPlaneIt)
    for (std::vector<art::Ptr<recob::Hit>>::iterator hitIt =
           std::next(showerHitsMap.at(*otherPlaneIt).begin());
  ...
  if (tpcCount.size() == 1 and
      tpcCount.begin()->first ==
        (int)(*std::next(showerHitsMap.at(problemPlane).begin(), nHits))
          ->WireID()
          .TPC) {
  ...
  std::map<int, std::vector<art::Ptr<recob::Hit>>> const& showerHitsMap) const
{
  ...
  if (showerHitsMap.size() < 2) {
    ...
    //  -- the 3D start point makes sense when projected back onto the
    //  individual planes
    ...
    TVector3 showerStartPos =
      Construct3DPoint_(startHits.at(0), startHits.at(1));
    TVector2 proj1 = Project3DPointOntoPlane_(showerStartPos, planes.at(0));
    TVector2 proj2 = Project3DPointOntoPlane_(showerStartPos, planes.at(1));

    double timingDifference =
      std::abs(startHits.at(0)->PeakTime() - startHits.at(1)->PeakTime());
    double projectionDifference =
      ((HitPosition_(*startHits.at(0)) - proj1).Mod() +
       (HitPosition_(*startHits.at(1)) - proj2).Mod()) /
      (double)2;

    if (timingDifference > 40 or projectionDifference > 5 or
        showerStartPos.X() == -9999 or showerStartPos.Y() == -9999 or
        showerStartPos.Z() == -9999)
      consistencyCheck = false;

    if (fDebug > 1)
      std::cout << "Timing difference is " << timingDifference
    ...
    //  -- the 3D start point formed by two views and projected back into the
    //  third is close to the start point in that view
    ...
    for (int plane = 0; plane < 3; ++plane) {
      ...
      TVector3 showerStartPos = Construct3DPoint_(
        start2DMap.at(otherPlanes.at(0)), start2DMap.at(otherPlanes.at(1)));
      TVector2 showerStartProj =
        Project3DPointOntoPlane_(showerStartPos, plane);

      if (fDebug > 2) {
        std::cout << "Plane... " << plane;
        std::cout << "\nStart position in this plane is "
                  << HitPosition_(*start2DMap.at(plane)).X() << ", "
                  << HitPosition_(*start2DMap.at(plane)).Y() << ")\n";
        std::cout << "Shower start from other two planes is ("
                  << showerStartPos.X() << ", " << showerStartPos.Y() << ", "
                  << showerStartPos.Z() << ")\n";
        std::cout << "Projecting the other two planes gives position ("
                  << showerStartProj.X() << ", " << showerStartProj.Y()
                  << ")\n";
      }

      double projDiff =
        std::abs((showerStartProj - HitPosition_(*start2DMap.at(plane))).Mod());
      double timeDiff =
        TMath::Max(std::abs(start2DMap.at(plane)->PeakTime() -
                            start2DMap.at(otherPlanes.at(0))->PeakTime()),
                   std::abs(start2DMap.at(plane)->PeakTime() -
                            start2DMap.at(otherPlanes.at(1))->PeakTime()));

      if (fDebug > 1)
        std::cout << "Plane " << plane << " has projDiff " << projDiff
                  << " and timeDiff " << timeDiff << '\n';
  ...
  if (fDebug > 1)
    std::cout << "Consistency check is " << consistencyCheck << '\n';
  ...
  art::FindManyP<recob::Hit> const& fmh) const
{
  ...
  // Look at each shower
  for (auto initialShowerIt = initialShowers.cbegin();
       initialShowerIt != initialShowers.cend();
       ++initialShowerIt) {

    if (std::distance(initialShowers.cbegin(), initialShowerIt) > 0)
      continue;
    ...
    for (auto const& [plane, clusterPtrs] : planeClusters) {
      for (auto const& clusterPtr : clusterPtrs) {
        chargeDist[plane] =
          new TH1D(std::string("chargeDist_Plane" + std::to_string(plane) +
        ...
        for (auto const& hit : hits | ranges::view::transform(to_element)) {
    ...
    // Look at how many clusters each plane has, and the proportion of hits each
    // one uses
    std::map<int, std::vector<double>> planeClusterSizes;
    for (std::map<int, std::vector<art::Ptr<recob::Cluster>>>::iterator
           planeClustersIt = planeClusters.begin();
      ...
      for (std::vector<art::Ptr<recob::Cluster>>::iterator planeClusterIt =
        ...
        planeClusterSizes[planeClustersIt->first].push_back(
          (double)hits.size() /
          (double)planeHits.at(planeClustersIt->first).size());
      }
    }
    ...
    for (auto const& [plane, cluster_sizes] : planeClusterSizes) {
      double const average = ranges::accumulate(cluster_sizes, 0.) / cluster_sizes.size();
      planeClustersAvSizes[plane] = average;
    }

    // Now decide if there is one plane which is ruining the reconstruction
    // If two planes have a low average cluster fraction and one high, this
    // plane likely merges two particle deposits together
    int badPlane = -1;
    for (auto const [plane, avg] : planeClustersAvSizes) {
      ...
      for (auto const [other_plane, other_avg] : planeClustersAvSizes)
        if (plane != other_plane)
          otherAverages.push_back(other_avg);

      double const sumSquareAvgsOtherPlanes = ranges::accumulate(otherAverages,
                                                                 0.,
                                                                 [](double sum, double avg) { return sum + cet::square(avg); });
      ...
      if (avg >= quadOtherPlanes)
        badPlane = plane;
    }

    if (badPlane != -1) {
      if (fDebug > 1)
        std::cout << "Bad plane is " << badPlane << '\n';
...
TVector3
shower::EMShowerAlg::Construct3DPoint_(art::Ptr<recob::Hit> const& hit1,
                                       art::Ptr<recob::Hit> const& hit2) const
{

  // x is average of the two x's
  double x =
    (fDetProp->ConvertTicksToX(hit1->PeakTime(), hit1->WireID().planeID()) +
     fDetProp->ConvertTicksToX(hit2->PeakTime(), hit2->WireID().planeID())) /
    (double)2;
...
shower::EMShowerAlg::ConstructTrack(
  ...
  std::map<int, TVector2> const& showerCentreMap) const
{
  ...
  if ((*hits1.begin())->WireID().TPC != (*hits2.begin())->WireID().TPC) {
    mf::LogWarning("EMShowerAlg")
      << "Warning: attempting to construct a track from two different TPCs.  "
         "Returning a null track.";
    return track;
  }

  // Check for tracks crossing TPC boundaries
  std::map<int, int> tpcMap;
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator hitIt = hits1.begin();
       hitIt != hits1.end();
       ++hitIt)
    ++tpcMap[(*hitIt)->WireID().TPC];
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator hitIt = hits2.begin();
       hitIt != hits2.end();
       ++hitIt)
    ++tpcMap[(*hitIt)->WireID().TPC];
  if (tpcMap.size() > 1) {
    mf::LogWarning("EMShowerAlg")
      << "Warning: attempting to construct a track which crosses more than one "
         "TPC -- PMTrack can't handle this right now.  Returning a track made "
         "just from hits in the first TPC it traverses.";
    unsigned int firstTPC1 = hits1.at(0)->WireID().TPC,
                 firstTPC2 = hits2.at(0)->WireID().TPC;
    for (std::vector<art::Ptr<recob::Hit>>::const_iterator hitIt =
           hits1.begin();
         hitIt != hits1.end();
         ++hitIt)
      if ((*hitIt)->WireID().TPC == firstTPC1)
        track1.push_back(*hitIt);
    for (std::vector<art::Ptr<recob::Hit>>::const_iterator hitIt =
           hits2.begin();
         hitIt != hits2.end();
         ++hitIt)
      if ((*hitIt)->WireID().TPC == firstTPC2)
        track2.push_back(*hitIt);
  } else {
    track1 = hits1;
    track2 = hits2;
  }
  ...
  if (fDebug > 1) {
    ...
    for (std::vector<art::Ptr<recob::Hit>>::const_iterator hit1 =
           track1.begin();
         hit1 != track1.end();
         ++hit1)
      std::cout << "Hit (" << HitCoordinates_(**hit1).X() << ", "
                << HitCoordinates_(**hit1).Y() << ") (real wire "
                << (*hit1)->WireID().Wire << ") in TPC "
                << (*hit1)->WireID().TPC << '\n';
    for (std::vector<art::Ptr<recob::Hit>>::const_iterator hit2 =
           track2.begin();
         hit2 != track2.end();
         ++hit2)
      std::cout << "Hit (" << HitCoordinates_(**hit2).X() << ", "
                << HitCoordinates_(**hit2).Y() << ") (real wire "
                << (*hit2)->WireID().Wire << ") in TPC "
                << (*hit2)->WireID().TPC << '\n';
  }

  TVector3 trackStart = Construct3DPoint_(track1.at(0), track2.at(0));
  pma::Track3D* pmatrack =
    fProjectionMatchingAlg.buildSegment(track1, track2, trackStart);

  if (!pmatrack) {
    mf::LogInfo("EMShowerAlg")
      << "Skipping this event because not enough hits in two views";
    return track;
  }

  std::vector<TVector3> xyz, dircos;
  // std::vector<std::vector<double> > dEdx; // Right now, not finding the dE/dx
  // for these tracks.  Can extend if needed.
  ...
  for (std::map<int, TVector2>::const_iterator showerCentreIt =
         showerCentreMap.begin();
       showerCentreIt != showerCentreMap.end();
       ++showerCentreIt) {

    // Project the vertex and the end point onto this plane
    TVector2 vertexProj =
      Project3DPointOntoPlane_(vertex, showerCentreIt->first);
    TVector2 endProj = Project3DPointOntoPlane_(end, showerCentreIt->first);

    // Find the distance of each to the centre of the cluster
    distanceToVertex[showerCentreIt->first] =
      (vertexProj - showerCentreIt->second).Mod();
    distanceToEnd[showerCentreIt->first] =
      (endProj - showerCentreIt->second).Mod();
  }

  // Find the average distance to the vertex and the end across the planes
  double avDistanceToVertex = 0, avDistanceToEnd = 0;
  for (std::map<int, double>::iterator distanceToVertexIt =
         distanceToVertex.begin();
  ...
  if (fDebug > 2)
    std::cout << "Distance to vertex is " << avDistanceToVertex
              << " and distance to end is " << avDistanceToEnd << '\n';

  // Change order if necessary
  if (avDistanceToEnd > avDistanceToVertex) {
    std::reverse(xyz.begin(), xyz.end());
    std::transform(dircos.begin(),
                   dircos.end(),
                   dircos.begin(),
                   [](TVector3 const& vec) { return -1 * vec; });
  }

  if (xyz.size() != dircos.size())
    mf::LogError("EMShowerAlg")
  ...
  std::vector<art::Ptr<recob::Hit>> const& track2) const
{
  ...
  return ConstructTrack(track1, track2, showerCentreMap);
}

double
shower::EMShowerAlg::FinddEdx_(
  ...
  std::unique_ptr<recob::Track> const& track) const
{
  ...
  double pitch = 0;
  try {
    pitch = lar::util::TrackPitchInView(*track, trackHits.at(0)->View());
  }
  catch (...) {}

  // Deal with large pitches
  if (pitch > fdEdxTrackLength) {
    return fCalorimetryAlg.dEdx_AREA(*trackHits.begin(), pitch);
  }

  for (auto const& hit : trackHits | ranges::view::transform(to_element)) {
  ...
  return
    fCalorimetryAlg.dEdx_AREA(dQdx, avHitTime, trackHits.at(0)->WireID().Plane);
}
  ...
  int plane) const
{
  ...
  if (fDebug > 1) {
    ...
    for (auto const& [plane, hitPtrs] : initialTrackHits) {
      std::cout << "  Plane " << plane << '\n';
      for (auto const& hit : hitPtrs | ranges::view::transform(to_element)) {
        std::cout << "    Hit is (" << HitCoordinates_(hit).X()
                  << " (real hit " << hit.WireID() << "), "
  ...
  initialTrack = MakeInitialTrack_(initialTrackHits, showerHitsMap);
  if (initialTrack and fDebug > 1) {
    std::cout << "The track start is (" << initialTrack->Vertex().X() << ", "
              << initialTrack->Vertex().Y() << ", "
              << initialTrack->Vertex().Z() << ")\n";
    std::cout << "The track direction is ("
              << initialTrack->VertexDirection().X() << ", "
              << initialTrack->VertexDirection().Y() << ", "
              << initialTrack->VertexDirection().Z() << ")\n";
  }
  if (fDebug > 1)
  ...
  bool perpendicular) const
{
  ...
  if (fMakeGradientPlot) {
    ...
    for (auto const& hit : showerHits | ranges::view::transform(to_element)) {
      int tpc = hit.WireID().TPC;
      if (graphs[tpc] == nullptr)
        graphs[tpc] = new TGraph();
      graphs[tpc]->SetPoint(graphs[tpc]->GetN(),
                            HitPosition_(hit).X(),
                            HitPosition_(hit).Y());
    }
...
shower::EMShowerAlg::FindShowers(
  std::map<int, std::vector<int>> const& trackToClusters) const
{
  ...
  for (auto const& clusters : trackToClusters | ranges::view::values) {
    ...
    //   mf::LogInfo("EMShowerAlg") << "Number of showers this track matches is
    //   " << matchingShowers.size() << '\n';

    // New shower
    if (matchingShowers.size() < 1)
      showers.push_back(clusters);
  ...
  const
{
  ...
  for (auto const& [plane, orderedShower] : orderedShowerMap) {
    ...
    for (auto const& hit : orderedShower | ranges::view::transform(to_element))
      wires.push_back(std::round(HitCoordinates_(hit).X()));

    cet::sort_all(wires);
    if (std::abs(*wires.begin() -
                 std::round(HitCoordinates_(**orderedShower.begin()).X())) >
          3 and
        std::abs(*wires.rbegin() -
                 std::round(HitCoordinates_(**orderedShower.begin()).X())) > 3)
      wireDirection = false;
    ...
    if (wireDirection) {
      ...
      for (auto const& hitPtr : orderedShower) {
        ...
        if (wireHitMap[wire].size() > 1) {
          ...
          continue;
                    ...
                    (wireHitMap[wire + 2].size() > 1 or
                     wireHitMap[wire + 3].size() > 1)) or
                   (!increasing and wireHitMap[wire - 1].size() > 1 and
                    (wireHitMap[wire - 2].size() > 1 or
                     wireHitMap[wire - 3].size() > 1))) {
          if ((increasing and (wireHitMap[wire + 4].size() < 2 and
                               wireHitMap[wire + 5].size() < 2 and
                               wireHitMap[wire + 6].size() < 2 and
                               wireHitMap[wire + 9].size() > 1)) or
              (!increasing and
               (wireHitMap[wire - 4].size() < 2 and
                wireHitMap[wire - 5].size() < 2 and
                wireHitMap[wire - 6].size() < 2) and
      ...
      if (!initialHits.size())
        initialHits.push_back(*orderedShower.begin());
    }
    ...
    // Deal with showers travelling along ticks
      ...
      for (auto const& hitPtr : orderedShower) {
        ...
        if ((increasing and
             (tickHitMap[tick + 1].size() or tickHitMap[tick + 2].size() or
    ...
    // Quality check -- make sure there isn't a single hit in a different TPC
    // (artifact of tracking failure)
    std::vector<art::Ptr<recob::Hit>> newInitialHits;
    std::map<int, int> tpcHitMap;
    std::vector<int> singleHitTPCs;
    for (auto const& hit : initialHits | ranges::view::transform(to_element))
      ++tpcHitMap[hit.WireID().TPC];

    for (auto const [tpc, count] : tpcHitMap)
      if (count == 1)
        singleHitTPCs.push_back(tpc);
  ...
  const std::map<int, std::vector<art::Ptr<recob::Hit>>>& showerHitsMap) const
{
  ...
  // Get the properties of the shower hits across the planes which will be used
  // to determine the likelihood of a particular reorientation permutation
  ...
  for (auto const& [plane, hitPtrs] : showerHitsMap) {
    planeRMS[plane] = ShowerHitRMS_(hitPtrs);
    planeRMSGradients[plane] = ShowerHitRMSGradient_(hitPtrs);
  }

  // Order these backwards so they can be used to discriminate between planes
  std::map<double, int> gradientMap;
  for (int const plane : showerHitsMap | ranges::view::keys)
  ...
  if (fDebug > 1)
    for (auto const [gradient, plane] : wireWidthMap)
      std::cout << "Plane " << plane
                ...
                << "; and an RMS gradient of "
                << planeRMSGradients[plane] << '\n';
  ...
  for (int const plane : wireWidthMap | ranges::view::values) {
    std::map<int, bool> permutation;
    permutation[plane] = true;
    for (int const plane2 : wireWidthMap | ranges::view::values)
      if (plane != plane2)
        permutation[plane2] = false;
  ...
  for (int const plane : wireWidthMap | ranges::view::reverse | ranges::view::values) {
    std::map<int, bool> permutation;
    permutation[plane] = false;
    for (int const plane2 : wireWidthMap | ranges::view::values)
      if (plane != plane2)
        permutation[plane2] = true;
  ...
  if (fDebug > 1) {
    ...
    for (auto const& [index, permutation] : permutations) {
      ...
      for (auto const [plane, value] : permutation)
        std::cout << "  Plane " << plane << " has value "
                  << value << '\n';
    }
  }
  ...
  std::map<int, std::vector<art::Ptr<recob::Hit>>> const& showerHitsMap) const
{
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::const_iterator
         initialHitIt = initialHitsMap.begin();
       initialHitIt != initialHitsMap.end();
       ++initialHitIt)
    initialHitsSize.push_back(
      std::make_pair(initialHitIt->first, initialHitIt->second.size()));

  // Sort the planes by number of hits
  std::sort(
    initialHitsSize.begin(),
    initialHitsSize.end(),
  ...
  if (initialHitsSize.size() > 2 and !CheckShowerHits_(showerHitsMap)) {
    int planeToIgnore = WorstPlane_(showerHitsMap);
    if (fDebug > 1)
      std::cout << "Igoring plane " << planeToIgnore
                << " in creation of initial track\n";
    for (std::vector<std::pair<int, int>>::iterator hitsSizeIt =
           initialHitsSize.begin();
  ...
  return ConstructTrack(initialHitsMap.at(planes.at(0)),
                        initialHitsMap.at(planes.at(1)));
}
  ...
  std::map<int, std::vector<art::Ptr<recob::Hit>>> const& initialHitsMap) const
{
  ...
  for (art::PtrVector<recob::Hit>::const_iterator hit = hits.begin();
       hit != hits.end();
       ++hit)
  ...
  for (unsigned int plane = 0; plane < fGeom->MaxPlanes(); ++plane) {
    ...
    if (planeHitsMap.count(plane)) {
      dEdx.push_back(FinddEdx_(initialHitsMap.at(plane), initialTrack));
      totalEnergy.push_back(
        fShowerEnergyAlg.ShowerEnergy(planeHitsMap.at(plane), plane));
      if (planeHitsMap.at(plane).size() > highestNumberOfHits and
          initialHitsMap.count(plane)) {
        bestPlane = plane;
  ...
  if (fDebug > 0) {
    ...
    std::cout << "\ndE/dx for each plane is: " << dEdx[0] << ", " << dEdx[1]
              << " and " << dEdx[2];
    std::cout << "\nTotal energy for each plane is: " << totalEnergy[0] << ", "
              << totalEnergy[1] << " and " << totalEnergy[2];
    std::cout << "\nThe shower start is (" << showerStart.X() << ", "
              << showerStart.Y() << ", " << showerStart.Z() << ")\n";
    std::cout << "The shower direction is (" << direction.X() << ", "
...
recob::Shower
shower::EMShowerAlg::MakeShower(art::PtrVector<recob::Hit> const& hits,
                                ...
                                int& iok) const
{
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator planeHits =
         planeHitsMap.begin();
       planeHits != planeHitsMap.end();
       ++planeHits) {

    std::vector<art::Ptr<recob::Hit>> showerHits;
    OrderShowerHits_(planeHits->second, showerHits, vertex);
    FindInitialTrackHits(
      showerHits, vertex, initialTrackHits[planeHits->first]);
  ...
  if (pl0 != -1 && pl1 != -1 && initialTrackHits[pl0].size() >= 2 &&
    ...
    pma::Track3D* pmatrack = fProjectionMatchingAlg.buildSegment(
      initialTrackHits[pl0], initialTrackHits[pl1]);
    ...
    if (spts.size() >= 2) { // at least two space points
      ...
      for (unsigned int plane = 0; plane < fGeom->MaxPlanes(); ++plane) {
        if (planeHitsMap.find(plane) != planeHitsMap.end()) {
          totalEnergy.push_back(
            fShowerEnergyAlg.ShowerEnergy(planeHitsMap[plane], plane));
        } else {
          totalEnergy.push_back(0);
        }
        if (initialTrackHits[plane].size()) {
          ...
          double pitch = 0;
          double wirepitch =
            fGeom->WirePitch(initialTrackHits[plane][0]->WireID().planeID());
          double angleToVert =
          ...
          if (pitch) {
            ...
            if (totQ) {
              ...
              fdEdx = fCalorimetryAlg.dEdx_AREA(
                dQdx, avgT / nhits, initialTrackHits[plane][0]->WireID().Plane);
            }
          }
  ...
  std::vector<std::vector<art::Ptr<recob::Hit>>>& hitAssns) const
{
  ...
  //  -- For each hit on the first plane, consider the 3D point made by
  //     combining with each hit from the second plane
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::const_iterator
         showerHitIt = showerHits.begin();
       showerHitIt != showerHits.end();
       ++showerHitIt) {

    // Find the other planes with hits
    std::vector<int> otherPlanes;
    for (unsigned int otherPlane = 0; otherPlane < fGeom->MaxPlanes();
         ++otherPlane)
      if ((int)otherPlane != showerHitIt->first and
          showerHits.count(otherPlane))
        otherPlanes.push_back(otherPlane);
    ...
    for (std::vector<art::Ptr<recob::Hit>>::const_iterator planeHitIt =
           showerHitIt->second.begin();
         planeHitIt != showerHitIt->second.end();
         ++planeHitIt) {

      if (std::find(usedHits.begin(), usedHits.end(), planeHitIt->key()) !=
          usedHits.end())
        continue;

      // Make a 3D point with every hit on the second plane
      const std::vector<art::Ptr<recob::Hit>> otherPlaneHits =
        showerHits.at(otherPlanes.at(0));
      for (std::vector<art::Ptr<recob::Hit>>::const_iterator otherPlaneHitIt =
           ...
           std::find(usedHits.begin(), usedHits.end(), planeHitIt->key()) ==
             usedHits.end();
           ++otherPlaneHitIt) {

        if ((*otherPlaneHitIt)->WireID().TPC != (*planeHitIt)->WireID().TPC or
            std::find(usedHits.begin(),
                      usedHits.end(),
                      otherPlaneHitIt->key()) != usedHits.end())
          continue;

        TVector3 point = Construct3DPoint_(*planeHitIt, *otherPlaneHitIt);
        std::vector<art::Ptr<recob::Hit>> pointHits;
        bool truePoint = false;

        if (otherPlanes.size() > 1) {

          TVector2 projThirdPlane =
            Project3DPointOntoPlane_(point, otherPlanes.at(1));
          const std::vector<art::Ptr<recob::Hit>> otherOtherPlaneHits =
            showerHits.at(otherPlanes.at(1));

          for (std::vector<art::Ptr<recob::Hit>>::const_iterator
                 otherOtherPlaneHitIt = otherOtherPlaneHits.begin();
               otherOtherPlaneHitIt != otherOtherPlaneHits.end() and !truePoint;
               ++otherOtherPlaneHitIt) {

            if ((*otherOtherPlaneHitIt)->WireID().TPC ==
                  (*planeHitIt)->WireID().TPC and
                (projThirdPlane - HitPosition_(**otherOtherPlaneHitIt)).Mod() <
                  fSpacePointSize) {
        ...
        else if ((Project3DPointOntoPlane_(point,
                                           (*planeHitIt)->WireID().Plane) -
                  HitPosition_(**planeHitIt))
                     .Mod() < fSpacePointSize and
                 (Project3DPointOntoPlane_(point,
                                           (*otherPlaneHitIt)->WireID().Plane) -
                  HitPosition_(**otherPlaneHitIt))
                     .Mod() < fSpacePointSize) {
  ...
  if (fDebug > 0) {
    ...
    if (fDebug > 1)
      for (std::vector<recob::SpacePoint>::const_iterator spacePointIt =
             spacePoints.begin();
           spacePointIt != spacePoints.end();
           ++spacePointIt) {
        const double* xyz = spacePointIt->XYZ();
        std::cout << "  Space point (" << xyz[0] << ", " << xyz[1] << ", "
                  << xyz[2] << ")\n";
      }
  }
...
std::map<int, std::vector<art::Ptr<recob::Hit>>>
shower::EMShowerAlg::OrderShowerHits(art::PtrVector<recob::Hit> const& shower,
                                     int desired_plane) const
{
  ...
  std::map<int, double> planeRMSGradients, planeRMS;
  for (auto const& [plane, hits] : showerHitsMap) {
    if (desired_plane != plane and desired_plane != -1)
      continue;
    std::vector<art::Ptr<recob::Hit>> orderedHits = FindOrderOfHits_(hits);
    planeRMS[plane] = ShowerHitRMS_(orderedHits);
    planeRMSGradients[plane] = ShowerHitRMSGradient_(orderedHits);
    showerHitsMap[plane] = orderedHits;
  }

  if (fDebug > 1) {
    for (auto const [plane, shower_hit_rms] : planeRMS) {
      std::cout << "Plane " << plane << " has RMS " << shower_hit_rms
                << " and RMS gradient " << planeRMSGradients.at(plane) << '\n';
    }
  }
  ...
  if (fDebug > 2) {
    ...
    for (auto const& [plane, hitPtrs] : showerHitsMap) {
      std::cout << "  Plane " << plane << '\n';
      for (auto const& hit : hitPtrs | ranges::view::transform(to_element)) {
        std::cout << "    Hit at (" << HitCoordinates_(hit).X() << ", "
                  << HitCoordinates_(hit).Y() << ") -- real wire "
                  << hit.WireID() << ", hit position (" << HitPosition_(hit).X()
                  << ", " << HitPosition_(hit).Y() << ")\n";
      }
    }
  }
  ...
  // Check between the views to make sure there isn't a poorly formed shower in
  // just one view First, determine the average RMS and RMS gradient across the
  // other planes
  std::map<int, double> planeOtherRMS, planeOtherRMSGradients;
  for (std::map<int, double>::iterator planeRMSIt = planeRMS.begin();
       planeRMSIt != planeRMS.end();
       ++planeRMSIt) {
    ...
    for (int plane = 0; plane < (int)fGeom->MaxPlanes(); ++plane) {
      if (plane != planeRMSIt->first and planeRMS.count(plane)) {
        ...
        planeOtherRMSGradients[planeRMSIt->first] +=
  ...
  for (auto const& [plane, hitPtrs] : showerHitsMap) {
    if (planeRMS.at(plane) > planeOtherRMS.at(plane) * 2) {
      ...
      std::vector<art::Ptr<recob::Hit>> orderedHits =
        this->FindOrderOfHits_(hitPtrs, true);
      showerHitsMap[plane] = orderedHits;
      planeRMSGradients[plane] = this->ShowerHitRMSGradient_(orderedHits);
    }
  }
  ...
  if (fDebug > 1) {
    ...
    for (auto const& [plane, hitPtrs] : showerHitsMap) {
      std::cout << "  Plane " << plane << " has start ("
                << HitCoordinates_(*hitPtrs.front()).X() << ", "
  ...
  if (fDebug > 2) {
    ...
    for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator
           showerHitsIt = showerHitsMap.begin();
         showerHitsIt != showerHitsMap.end();
         ++showerHitsIt) {
      std::cout << "  Plane " << showerHitsIt->first << '\n';
      for (std::vector<art::Ptr<recob::Hit>>::iterator hitIt =
             showerHitsIt->second.begin();
           hitIt != showerHitsIt->second.end();
           ++hitIt)
        std::cout << "    Hit at (" << HitCoordinates_(**hitIt).X() << ", "
                  << HitCoordinates_(**hitIt).Y() << ") -- real wire "
                  << (*hitIt)->WireID() << ", hit position ("
                  << HitPosition_(**hitIt).X() << ", "
                  << HitPosition_(**hitIt).Y() << ")\n";
    }
  }
  ...
  if (fDebug > 1) {
    ...
    for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator
      ...
      std::cout << "  Plane " << showerHitsIt->first << " has start ("
                ...
                << HitCoordinates_(*showerHitsIt->second.front()).Y()
  ...
  if (fDebug > 1)
    std::cout << "Here are the relative wire widths... \n";
  for (auto const [relative_wire_width, plane] : wireWidths) {
    if (fDebug > 1)
      std::cout << "Plane " << plane
                << " has relative wire width " << relative_wire_width << '\n';
    if (relative_wire_width < 1 / (double)wireWidths.size())
      badPlanes.push_back(plane);
  }
  ...
  if (badPlanes.size() == 1) {
    ...
    if (fDebug > 1)
      std::cout << "Ignoring plane " << badPlane << " when orientating\n";
  ...
  if (!CheckShowerHits_(showerHitsMap))
  ...
  if (fDebug > 2) {
    ...
    for (auto const& [plane, hitPtrs] : showerHitsMap) {
      std::cout << "  Plane " << plane << '\n';
      for (auto const& hit : hitPtrs | ranges::view::transform(to_element)) {
        std::cout << "    Hit (" << HitCoordinates_(hit).X() << " (real wire "
                  << hit.WireID() << "), " << HitCoordinates_(hit).Y()
                  << ") -- pos (" << HitPosition_(hit).X() << ", "
                  << HitPosition_(hit).Y() << ")\n";
      }
    }
  }
...
void
shower::EMShowerAlg::FindInitialTrackHits(
      ...
      if (hit->WireID().TPC == tpc.TPC) {
        ...
        if (i == 0 ||
            (std::abs((coord.Y() - (parm[0] + coord.X() * parm[1])) *
                      cos(atan(parm[1]))) < fToler[i - 1]) ||
            fitok == 1) {
          ++nhits;
          if (nhits == fNfithits[i] + 1)
            break;
...
TVector2
shower::EMShowerAlg::HitPosition_(recob::Hit const& hit) const
{

  geo::PlaneID planeID = hit.WireID().planeID();

  return HitPosition_(HitCoordinates_(hit), planeID);
}

TVector2
shower::EMShowerAlg::HitPosition_(TVector2 const& pos,
  ...
  if (fGeom->SignalType(wireID) == geo::kInduction) {
    ...
    if (wireID.TPC % 2 == 0)
      globalWire = fGeom->WireCoordinate(
        wireCentre[1], wireCentre[2], wireID.Plane, 0, wireID.Cryostat);
    else
      globalWire = fGeom->WireCoordinate(
        wireCentre[1], wireCentre[2], wireID.Plane, 1, wireID.Cryostat);
  }

  // Collection
  else {
    // FOR COLLECTION WIRES, HARD CODE THE GEOMETRY FOR GIVEN DETECTORS
    // THIS _SHOULD_ BE TEMPORARY. GLOBAL WIRE SUPPORT IS BEING ADDED TO THE
    // LARSOFT GEOMETRY AND SHOULD BE AVAILABLE SOON
    if (fDetector == "dune35t") {
      ...
      if (wireID.TPC == 0 or wireID.TPC == 1)
      ...
      else if (wireID.TPC == 2 or wireID.TPC == 3 or wireID.TPC == 4 or
               wireID.TPC == 5)
        ...
        mf::LogError("BlurredClusterAlg")
          << "Error when trying to find a global induction plane coordinate "
             "for TPC "
          << wireID.TPC << " (geometry" << fDetector << ")";
    } else if (fDetector == "dune10kt") {
      unsigned int nwires = fGeom->Nwires(wireID.Plane, 0, wireID.Cryostat);
      // Detector geometry has four TPCs, two on top of each other, repeated
      // along z...
  ...
  const std::map<int, std::vector<art::Ptr<recob::Hit>>>& showerHitsMap) const
{
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::const_iterator
       ...
       showerHitsIt != showerHitsMap.end();
       ++showerHitsIt)
    wireWidthMap[planeOtherWireLengths.at(showerHitsIt->first)] =
      showerHitsIt->first;
...
TVector2
shower::EMShowerAlg::ShowerDirection_(
  const std::vector<art::Ptr<recob::Hit>>& showerHits) const
{
  ...
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator hit =
         showerHits.begin();
       hit != showerHits.end();
       ++hit) {
    //++nhits;
    pos = HitPosition_(**hit);
  ...
  double gradient =
    (sumweight * sumxy - sumx * sumy) / (sumweight * sumx2 - sumx * sumx);
...
TVector2
shower::EMShowerAlg::ShowerCenter_(
  const std::vector<art::Ptr<recob::Hit>>& showerHits) const
{
...
double
shower::EMShowerAlg::ShowerHitRMS_(
  const std::vector<art::Ptr<recob::Hit>>& showerHits) const
{

  TVector2 direction = ShowerDirection_(showerHits);
  TVector2 centre = ShowerCenter_(showerHits);

  std::vector<double> distanceToAxis;
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator showerHitsIt =
         showerHits.begin();
       showerHitsIt != showerHits.end();
       ++showerHitsIt) {
    TVector2 proj =
      (HitPosition_(**showerHitsIt) - centre).Proj(direction) + centre;
    distanceToAxis.push_back((HitPosition_(**showerHitsIt) - proj).Mod());
  }
  ...
  TVector2 trueStart) const
{
  ...
  double lengthOfShower =
    (HitPosition_(*showerHits.back()) - HitPosition_(*showerHits.front()))
      .Mod();
  ...
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator showerHitsIt =
       ...
       showerHitsIt != showerHits.end();
       ++showerHitsIt) {
    showerSegments[(int)(HitPosition_(**showerHitsIt) -
                         HitPosition_(*showerHits.front()))
                     .Mod() /
                   lengthOfSegment]
      .push_back(*showerHitsIt);
    segmentCharge[(int)(HitPosition_(**showerHitsIt) -
                        HitPosition_(*showerHits.front()))
                    .Mod() /
                  lengthOfSegment] += (*showerHitsIt)->Integral();
  }
  ...
  // progresses
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator
         showerSegmentIt = showerSegments.begin();
       showerSegmentIt != showerSegments.end();
       ++showerSegmentIt) {

    // Get the mean position of the hits in this bin
    TVector2 meanPosition(0, 0);
    for (std::vector<art::Ptr<recob::Hit>>::iterator hitInSegmentIt =
           showerSegmentIt->second.begin();
         hitInSegmentIt != showerSegmentIt->second.end();
         ++hitInSegmentIt)
      meanPosition += HitPosition_(**hitInSegmentIt);
    ...
    for (std::vector<art::Ptr<recob::Hit>>::iterator hitInSegmentIt =
         ...
         ++hitInSegmentIt) {
      TVector2 proj =
        (HitPosition_(**hitInSegmentIt) - meanPosition).Proj(direction) +
        meanPosition;
      distanceToAxisBin.push_back(
        (HitPosition_(**hitInSegmentIt) - proj).Mod());
    }

    double RMSBin =
      TMath::RMS(distanceToAxisBin.begin(), distanceToAxisBin.end());
  ...
  double sumx = 0., sumy = 0., sumx2 = 0., sumxy = 0., sumweight = 0.;
  for (std::vector<std::pair<int, double>>::iterator binVsRMSIt =
         binVsRMS.begin();
       binVsRMSIt != binVsRMS.end();
       ++binVsRMSIt) {
...
TVector2
shower::EMShowerAlg::Project3DPointOntoPlane_(TVector3 const& point,
                                              int plane,
                                              int cryostat) const
{
  ...
  wireTickPos = TVector2(GlobalWire_(wireID),
                         fDetProp->ConvertXToTicks(point.X(), planeID));

  return HitPosition_(wireTickPos, planeID);
}
  ...
  if (fDebug > 1)
    for (auto const [plane, relative_width] : planeOtherWireLengths) {
      std::cout << "Plane " << plane << " has " << planeWireLength[plane]
                << " wire width and therefore has relative width in wire of "
                << relative_width << '\n';
    }
                                 ...
                                 Double_t* parm) const
{
  ...
  if (sumx2 * sumw - sumx * sumx == 0.)
    return 1;
  if (sumx2 - sumx * sumx / sumw == 0.)
    return 1;
...
bool
shower::EMShowerAlg::isCleanShower(
...
shower::HitPosition::HitPosition(geo::GeometryCore const* geom,
                                 detinfo::DetectorProperties const* detProp)
  : fGeom(geom), fDetProp(detProp)
{}
...
shower::HitPosition::HitPosition(TVector2 wiretick,
                                 ...
                                 detinfo::DetectorProperties const* detProp)
  : WireTick(wiretick), Cm(cm), fGeom(geom), fDetProp(detProp)
{}
...
shower::HitPosition::HitPosition(TVector2 wiretick,
                                 ...
                                 geo::GeometryCore const* geom,
                                 detinfo::DetectorProperties const* detProp)
  : HitPosition{wiretick, ConvertWireTickToCm(wiretick, planeID), geom, detProp}
{}
