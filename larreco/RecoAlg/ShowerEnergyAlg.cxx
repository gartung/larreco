...
shower::ShowerEnergyAlg::ShowerEnergyAlg(fhicl::ParameterSet const& pset)
  ...
  , fLinearFunctions{
      {{pset.get<double>("UGradient"), pset.get<double>("UIntercept")},
...
{
  return ShowerEnergy(fDetProp, hits, plane);
}
  ...
  geo::PlaneID::PlaneID_t const plane) const
{
  ...
  if (plane >= fLinearFunctions.size()) {
    return 0.;
  }

  auto const coeff =
    detprop.SamplingRate() / (detprop.ElectronLifetime() * 1e3);

  auto in_plane = [plane](auto const& hit) {
  ...
  double const totalCharge = ranges::accumulate(
    hits | ranges::view::transform(to_element) | ranges::view::filter(in_plane) |
      ranges::view::transform(charge),
    0.);
