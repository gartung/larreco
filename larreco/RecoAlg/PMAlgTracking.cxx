...
pma::convertFrom(const pma::Track3D& src, unsigned int tidx, int pdg)
{
  ...
  return recob::Track(
    recob::TrackTrajectory(
      std::move(positions), std::move(momenta), std::move(outFlags), false),
    pdg,
    totChi2,
    ndof,
    covStart,
...
pma::PMAlgTrackingBase::PMAlgTrackingBase(
  const std::vector<art::Ptr<recob::Hit>>& allhitlist,
...
void
pma::PMAlgTrackingBase::guideEndpoints(pma::TrkCandidateColl& tracks)
{
  for (auto const& t : tracks.tracks()) {
    ...
    if ((tpc == trk.BackTPC()) && (cryo == trk.BackCryo())) {
      ...
      fProjectionMatchingAlg.guideEndpoints(
        trk, pma::Track3D::kBegin, fHitMap[trk.FrontCryo()][trk.FrontTPC()]);
      fProjectionMatchingAlg.guideEndpoints(
        trk, pma::Track3D::kEnd, fHitMap[trk.BackCryo()][trk.BackTPC()]);
    }
  }
}
// ------------------------------------------------------
...
{
  mf::LogVerbatim("PMAlgFitter")
    << "Found " << allhitlist.size() << "hits in the event.";
  mf::LogVerbatim("PMAlgFitter")
  ...
  mf::LogVerbatim("PMAlgFitter")
    << "...done, " << fCluHits.size() << " clusters from "
...
pma::PMAlgFitter::buildTracks(void)
{
  ...
  for (const auto& pfpCluEntry : fPfpClusters) {
    ...
    // if (pdg == 11) continue;
    if (skipPdg && has(fTrackingSkipPdg, pdg))
      continue;
    if (selectPdg && !has(fTrackingOnlyPdg, pdg))
      continue;

    mf::LogVerbatim("PMAlgFitter")
      << "Process clusters from PFP:" << pfPartIdx << ", pdg:" << pdg;
    ...
    if (clu_count.size() >
        1) // try building only if there are clusters from multiple views
    {
      ...
      if (candidate.IsValid() && candidate.Track()->HasTwoViews() &&
        ...
        if (!std::isnan(candidate.Track()->Length())) {
  ...
  for (const auto& pfpCluEntry : fPfpClusters) {
        ...
        1) // try building only if there are clusters from multiple views
    {
      ...
      if (search != fPfpVtx.end()) {
        candidate.SetTrack(
          fProjectionMatchingAlg.buildShowerSeg(allHits, search->second));
  ...
  , fDetProp(lar::providerFrom<detinfo::DetectorPropertiesService>())
{
  ...
  mf::LogVerbatim("PMAlgTracker")
    << "Validation mode in config: " << pmalgTrackerConfig.Validation();
  ...
  if (pmalgTrackerConfig.Validation() == "hits") {
    ...
    throw cet::exception("pma::PMAlgTracker")
...
pma::PMAlgTracker::init(const art::FindManyP<recob::Hit>& hitsFromClusters)
{
  ...
  mf::LogVerbatim("PMAlgTracker")
    << "...done, " << fCluHits.size() << " clusters for 3D tracking.";
}
// ------------------------------------------------------
                        ...
                        const art::FindManyP<recob::Hit>& hitsFromEmParts)
{
  ...
  mf::LogVerbatim("PMAlgTracker")
    << "...done, " << n << " clusters for 3D tracking.";
}
// ------------------------------------------------------

double
pma::PMAlgTracker::validate(pma::Track3D& trk, unsigned int testView)
{
...
bool
pma::PMAlgTracker::reassignHits_1(const std::vector<art::Ptr<recob::Hit>>& hits,
                                  ...
                                  double dist2)
{
  ...
  bool result = false;
  if ((hits.size() > 1) ||
      (dist2 > 1.0)) // min. 2 hits or single hit separated from the rest
  {
    ...
    if (best_trk && (n_max >= hits.size() / 3)) // /2
    {
      mf::LogVerbatim("PMAlgTrackMaker")
        << "  Reassign(v1) " << n_max << " hits." << std::endl;
      ...
      pma::Track3D* ext =
        fProjectionMatchingAlg.extendTrack(*best_trk, hits, false);
      ext->SortHits();
      ext->ShiftEndsToHits();
      if (fProjectionMatchingAlg.isContained(*ext)) {
        tracks[best_u].SetTrack(
          ext); // and this deletes best_trk stored at best_u
        result = true;
      } else
        delete ext;
    } else if (hits.size() >= fMinSeedSize2ndPass) {
      size_t minSizeCompl =
        hits.size() / 8; // much smaller minimum required in complementary views
      if (minSizeCompl < 3)
        minSizeCompl = 3; // but at least three hits!
      ...
      pma::TrkCandidate candidate =
        matchCluster(-1, hits, minSizeCompl, tpc, cryo, first_view);
...
double
pma::PMAlgTracker::collectSingleViewFront(
  pma::Track3D& trk,
  std::vector<art::Ptr<recob::Hit>>& hits) const
{
...
bool
pma::PMAlgTracker::reassignSingleViewEnds_1(pma::TrkCandidateColl& tracks)
{
  bool result = false;
...
bool
pma::PMAlgTracker::reassignHits_2(const std::vector<art::Ptr<recob::Hit>>& hits,
                                  pma::TrkCandidateColl& tracks,
                                  size_t trk_idx,
                                  double dist2) const
{
      ...
      (dist2 > 1.0)) // min. 2 hits or single hit separated from the rest
  {
    ...
    {
      mf::LogVerbatim("PMAlgTrackMaker")
        << "  Reassign(v2) " << n_max << " hits." << std::endl;
...
bool
pma::PMAlgTracker::reassignSingleViewEnds_2(pma::TrkCandidateColl& tracks) const
{
  bool result = false;
  for (size_t t = 0; t < tracks.size(); t++) {
    pma::Track3D& trk = *(tracks[t].Track());
                               ...
                               double cosThr) const
{
  ...
  unsigned int k = 0;
  double distFF =
    pma::Dist2(trk1->front()->Point3D(), trk2->front()->Point3D());
  dist = distFF;
  ...
  if (dist < d) {
    ...
    switch (k) // swap or flip to get trk1 end before trk2 start
    {
      ...
      case 3:
        ...
        break;
      default:
        mf::LogError("PMAlgTracker") << "Should never happen.";
    }
    if (k == 1)
      reverseOrder = true;
    else
    ...
    TVector3 proj1 =
      pma::GetProjectionToSegment(endpoint1, trk2front0, trk2front1);
    ...
    TVector3 proj2 =
      pma::GetProjectionToSegment(endpoint2, trk1back1, trk1back0);
    ...
    if ((cos3d > cosThr) && (distProj1 < distProjThr) &&
        (distProj2 < distProjThr))
      return true;
    else // check if parallel to wires & colinear in 2D
    {
      ...
      if ((fabs(dir1_xz.Z()) > maxCosXZ) && (fabs(dir2_xz.Z()) > maxCosXZ)) {
        ...
        if ((cosXZ > cosThrXZ) && (distProj1 < distProjThrXZ) &&
            (distProj2 < distProjThrXZ))
          return true;
      }
    }
  }
...
bool
pma::PMAlgTracker::mergeCoLinear(pma::TrkCandidateColl& tracks) const
{
  ...
  std::sort(
    tracks.tracks().begin(), tracks.tracks().end(), pma::bTrack3DLonger());
  ...
  while (t < tracks.size()) {
    ...
    for (u = t + 1; u < tracks.size(); u++) {
      trk2 = tracks[u].Track();
      if (areCoLinear(
            trk1, trk2, d, c, r, distThr, distThrMin, distProjThr, cosThr)) {
        l = std::sqrt(
          pma::Dist2(trk2->front()->Point3D(), trk2->back()->Point3D()));
...
void
pma::PMAlgTracker::mergeCoLinear(pma::tpc_track_map& tracks) const
{
  double distThr = 0.25;   // max gap as a fraction of the longer track length
  ...
  for (auto& tpc_entry1 : tracks) {
    ...
    while (t < tracks1.size()) {
          ...
          for (size_t u = 0; u < tracks2.size(); u++) {
            ...
            if ((dfront2 < wallDistThr) || (dback2 < wallDistThr)) {
              if (areCoLinear(trk1,
                              ...
                              distProjThr,
                              cosThr)) {
                l = std::sqrt(pma::Dist2(trk2->front()->Point3D(),
                                         trk2->back()->Point3D()));
                if (((c > cmax) && (d < dmin + 0.5 * lbest)) ||
                    (0.75 * l < dmin)) {
                  cmax = c;
      ...
      if (best_trk2) {
        mf::LogVerbatim("PMAlgTracker")
          << "Merge track (" << tpc1 << ":" << tracks1.size() << ":"
          << trk1->size() << ") with track (" << best_tpc << ":"
          << tracks[best_tpc].size() << ":" << best_trk2->size() << ")";
        auto const* geom = lar::providerFrom<geo::Geometry>();
        const geo::TPCGeo& tpc1 = geom->TPC(trk1->Nodes().front()->TPC(),
                                            trk1->Nodes().front()->Cryo());
        const geo::TPCGeo& tpc2 = geom->TPC(best_trk2->Nodes().front()->TPC(),
                                            best_trk2->Nodes().front()->Cryo());
        if (reverse) {
...
pma::PMAlgTracker::build(void)
{
  ...
  for (auto tpc_iter = fGeom->begin_TPC_id(); tpc_iter != fGeom->end_TPC_id();
       tpc_iter++) {
    mf::LogVerbatim("PMAlgTracker")
    ...
    if (fValidation !=
        pma::PMAlgTracker::kHits) // initialize ADC images for all planes in
                                  // this TPC (in "adc" and "calib")
    {
      ...
      if (ok) {
        mf::LogVerbatim("PMAlgTracker") << "  ...done.";
      } else {
    ...
    fromMaxCluster_tpc(tracks[tpc_iter->TPC],
                       fMinSeedSize1stPass,
                       tpc_iter->TPC,
                       tpc_iter->Cryostat);
  ...
  if (fRunVertexing) {
    ...
    // reassignSingleViewEnds(result); // final check for correct hit-track
    // assignments
  }
  ...
  if (fFlipToBeam)
    fResult.flipTreesToCoordinate(
      2); // flip the tracks / trees to the beam direction (Z)
  else if (fFlipDownward)
    fResult.flipTreesToCoordinate(
      1); // flip the tracks / trees to point downward (-Y)
  else if (fFlipToX)
    fResult.flipTreesToCoordinate(0); // flip the tracks / trees to point in -X
                                      // direction (downwards for dual phase)

  if (fAutoFlip_dQdx)
    fResult
      .flipTreesByDQdx(); // flip the tracks / trees to get best dQ/dx sequences
...
void
pma::PMAlgTracker::fromMaxCluster_tpc(pma::TrkCandidateColl& result,
                                      size_t minBuildSize,
                                      unsigned int tpc,
                                      unsigned int cryo)
{
  ...
  size_t minSizeCompl =
    minBuildSize / 8; // smaller minimum required in complementary views
  if (minSizeCompl < 2)
    minSizeCompl = 2; // but at least two hits!

  int max_first_idx = 0;
  while (max_first_idx >=
         0) // loop over clusters, any view, starting from the largest
  {
    mf::LogVerbatim("PMAlgTracker") << "Find max cluster...";
    max_first_idx = maxCluster(minBuildSize,
                               geo::kUnknown,
                               tpc,
                               cryo); // any view, but must be track-like
    if ((max_first_idx >= 0) && !fCluHits[max_first_idx].empty()) {
      geo::View_t first_view = fCluHits[max_first_idx].front()->View();

      pma::TrkCandidate candidate =
        matchCluster(max_first_idx, minSizeCompl, tpc, cryo, first_view);
  ...
  geo::View_t first_view)
{
  ...
  unsigned int nFirstHits = first_hits.size(),
               first_plane_idx = first_hits.front()->WireID().Plane;
  mf::LogVerbatim("PMAlgTracker") << std::endl << "--- start new candidate ---";
  mf::LogVerbatim("PMAlgTracker")
    << "use view  *** " << first_view << " *** plane idx " << first_plane_idx
    << " ***  size: " << nFirstHits;

  float x,
    xmax = fDetProp->ConvertTicksToX(
      first_hits.front()->PeakTime(), first_plane_idx, tpc, cryo),
    xmin = xmax;
  // mf::LogVerbatim("PMAlgTracker") << "  *** x max0: " << xmax;
  for (size_t j = 1; j < first_hits.size(); ++j) {
    x = fDetProp->ConvertTicksToX(
      first_hits[j]->PeakTime(), first_plane_idx, tpc, cryo);
    if (x > xmax) {
      xmax = x;
    }
    if (x < xmin) {
  ...
  // mf::LogVerbatim("PMAlgTracker") << "  *** x max: " << xmax << " min:" <<
  // xmin;

  pma::TrkCandidateColl
    candidates; // possible solutions of the selected cluster and clusters in
                // complementary views
  ...
  while (try_build) // loop over complementary views
  {
    pma::TrkCandidate candidate;
    if (first_clu_idx >= 0)
      candidate.Clusters().push_back((size_t)first_clu_idx);
    ...
    for (auto av : fAvailableViews) {
      ...
      av_idx = maxCluster(
        first_clu_idx, candidates, xmin, xmax, minSizeCompl, av, tpc, cryo);
      if (av_idx >= 0) {
    ...
    if (try_build) {
      ...
      mf::LogVerbatim("PMAlgTracker") << "    cluster in view  *** " << bestView
                                      << " ***  size: " << nMaxHits;
      ...
      candidate.SetTrack(
        fProjectionMatchingAlg.buildTrack(first_hits, fCluHits[idx]));

      if (candidate
            .IsValid() && // no track if hits from 2 views do not alternate
          fProjectionMatchingAlg.isContained(*(candidate.Track()),
                                             2.0F)) // sticks out of TPC's?
      {
        m0 = candidate.Track()->GetMse();
        if (m0 < mseThr) // check validation only if MSE is OK - thanks for
                         // Tracy for noticing this
        {
          v0 = validate(*(candidate.Track()), testView);
        }
      }

      if (candidate.Track() && (m0 < mseThr) &&
          (v0 > validThr)) // good candidate, try to extend it
      {
        mf::LogVerbatim("PMAlgTracker")
        ...
        idx = 0;
        while (idx >= 0) // try to collect matching clusters, use **any** plane
                         // except validation
        {
          idx = matchCluster(
            candidate, minSize, fraction, geo::kUnknown, testView, tpc, cryo);
          if (idx >= 0) {
        ...
        while ((idx >= 0) &&
               (testView !=
                geo::kUnknown)) { //                     match clusters from the
                                  //                     plane used previously
                                  //                     for the validation
          idx = matchCluster(
            candidate, minSize, fraction, testView, geo::kUnknown, tpc, cryo);
          if (idx >= 0) {
        ...
        // validation:
        if (extended)
          candidate.SetValidation(validate(*(candidate.Track()), testView));
      } else {
        mf::LogVerbatim("PMAlgTracker")
          << "track REJECTED, MSE = " << m0 << "; v = " << v0;
  ...
  if (!candidates
         .empty()) // return best candidate, release other tracks and clusters
  {
    int best_trk = -1;
    ...
    for (size_t t = 0; t < candidates.size(); t++)
      if (candidates[t].IsGood() &&
        ...
        if ((f > max_f) ||
            ((f == max_f) && ((candidates[t].Validation() > max_v) ||
    ...
    if ((best_trk > -1) && candidates[best_trk].IsGood() &&
...
bool
pma::PMAlgTracker::extendTrack(pma::TrkCandidate& candidate,
                               ...
                               bool add_nodes)
{
  ...
  if (m_max < 0.05)
    m_max = 0.05; // this is still good, low MSE value

  double v_min1 = 0.98 * candidate.Validation();
  double v_min2 = 0.9 * candidate.Validation();

  pma::Track3D* copy =
    fProjectionMatchingAlg.extendTrack(*(candidate.Track()), hits, add_nodes);
...
int
pma::PMAlgTracker::matchCluster(const pma::TrkCandidate& trk,
                                size_t minSize,
                                double fraction,
...
int
pma::PMAlgTracker::maxCluster(int first_idx_tag,
                              ...
                              unsigned int cryo) const
{
  ...
  for (size_t i = 0; i < fCluHits.size(); ++i) {
    if ((fCluHits[i].size() < min_clu_size) ||
        (fCluHits[i].front()->View() != view) || has(fUsedClusters, i) ||
    ...
    if ((v.front()->WireID().TPC == tpc) &&
      ...
      for (size_t j = 0; j < v.size(); ++j) {
        x = fDetProp->ConvertTicksToX(
          v[j]->PeakTime(), v[j]->WireID().Plane, tpc, cryo);
        if ((x >= xmin) && (x <= xmax))
          s++;
      }
                              ...
                              unsigned int cryo) const
{
  ...
  for (size_t i = 0; i < fCluHits.size(); ++i) {
    ...
    if (v.empty() || (fCluWeights[i] < fTrackLikeThreshold) ||
        has(fUsedClusters, i) || has(fInitialClusters, i) ||
        has(fTriedClusters[view], i) ||
...
pma::PMAlgTracker::listUsedClusters(void) const
{
  ...
  for (size_t i = 0; i < fCluHits.size(); ++i) {
    if (!fCluHits[i].empty() && has(fUsedClusters, i)) {
      mf::LogVerbatim("PMAlgTracker")
        ...
        << ";\tsize: " << fCluHits[i].size() << ";\tweight: " << fCluWeights[i];
  ...
  for (size_t i = 0; i < fCluHits.size(); ++i) {
    if (!fCluHits[i].empty() && !has(fUsedClusters, i)) {
      ...
      //        h->Integral() << " "
      //        << fCluHits[i].size() << " " << i << " "
      //		<< fCluWeights[i] << " " << matchTrack(fResult,
      //fCluHits[i])/(float)fCluHits[i].size()
      //		<< std::endl;
      //}
    }
  }
