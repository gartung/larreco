...
{
  std::sort(
    fAssignedHits.begin(), fAssignedHits.end(), pma::bTrajectory3DOrderLess());
}
...
pma::Element3D::UpdateHitParams()
{
  ...
  for (auto h : fAssignedHits) {
    if (h->IsEnabled())
      fNThisHitsEnabledAll++;
    switch (h->View2D()) {
      case geo::kZ:
        hitsColl.push_back(h);
        break;
      case geo::kV:
        hitsInd2.push_back(h);
        break;
      case geo::kU:
        hitsInd1.push_back(h);
        break;
    }
  }
  ...
  pma::SortedObjectBase const* chain =
    dynamic_cast<pma::SortedObjectBase*>(this);
  pma::Element3D* el = 0;
...
pma::Element3D::SumDist2() const
{
  ...
  if (fAssignedPoints.size()) {
    ...
    for (auto p : fAssignedPoints) {
      d = sqrt(GetDistance2To(*p)) -
          0.5; // guide by ref points up to ~ 3D resolution
      if (d > 0.0)
        ref_sum += d * d;
    }
