...
namespace pma {
  typedef ROOT::Math::DisplacementVector2D<ROOT::Math::Cartesian2D<double>>
    Vector2D;
  ...
  double GetSummedADC(const std::vector<pma::Hit3D*>& hits,
                      unsigned int view = geo::kUnknown);
  double GetSummedAmpl(const std::vector<pma::Hit3D*>& hits,
                       unsigned int view = geo::kUnknown);

  double GetHitsRadius3D(const std::vector<pma::Hit3D*>& hits,
                         bool exact = false);
  double GetHitsRadius2D(const std::vector<pma::Hit3D*>& hits,
                         bool exact = false);

  double GetSegmentProjVector(const TVector2& p,
                              const TVector2& p0,
                              const TVector2& p1);
  double GetSegmentProjVector(const Vector2D& p,
                              const Vector2D& p0,
                              const Vector2D& p1);

  double GetSegmentProjVector(const TVector3& p,
                              const TVector3& p0,
                              const TVector3& p1);
  double GetSegmentProjVector(const Vector3D& p,
                              const Vector3D& p0,
                              const Vector3D& p1);

  TVector2 GetProjectionToSegment(const TVector2& p,
                                  const TVector2& p0,
                                  const TVector2& p1);
  TVector3 GetProjectionToSegment(const TVector3& p,
                                  const TVector3& p0,
                                  const TVector3& p1);

  double SolveLeastSquares3D(
...
struct pma::bTrack3DLonger
  : public std::binary_function<const pma::TrkCandidate&,
                                const pma::TrkCandidate&,
                                bool> {
