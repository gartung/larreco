 ...
 *
 *          Base for 3D segments and track nodes. See PmaTrack3D.h file for
 * details. See PmaTrack3D.h file for details.
 */
...
public:
  ...
  /// wire views.
  virtual double GetDistance2To(const TVector2& p2d,
                                unsigned int view) const = 0;

  /// Get 3D direction cosines corresponding to this element.
  virtual pma::Vector3D GetDirection3D() const = 0;

  virtual TVector3 GetUnconstrainedProj3D(const TVector2& p2d,
                                          unsigned int view) const = 0;
  ...
  bool
  HasHit(const pma::Hit3D* h) const
  {
  ...
  size_t
  NPrecalcEnabledHits() const
  {
    return fNThisHitsEnabledAll;
  }

  TVector3 const&
  ReferencePoint(size_t index) const
  {
  ...
  double
  SumHitsQ(unsigned int view) const
  {
    return fSumHitsQ[view];
  }
  unsigned int
  NHits(unsigned int view) const
  {
    return fNHits[view];
  }
  unsigned int
  NThisHits(unsigned int view) const
  {
  ...
  static float
  OptFactor(unsigned int view)
  {
    return fOptFactors[view];
  }
  static void
  SetOptFactor(unsigned int view, float value)
  {
