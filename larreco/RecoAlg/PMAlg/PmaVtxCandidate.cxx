 ...
 *
 *          Candidate for 3D vertex. Used to test intersections and join tracks
 * in vertices. See PmaTrack3D.h file for details.
 */
...
pma::VtxCandidate::IsAttached(pma::Track3D* trk) const
{
  ...
  if (!rootTrk)
    throw cet::exception("pma::VtxCandidate") << "Broken track.";
  ...
  for (const auto& t : fAssigned) {
    ...
    if (rootTrk->IsAttachedTo(rootAssn))
      return true;
  }
  return false;
}
...
pma::VtxCandidate::Size(double minLength) const
{
  ...
  for (auto const& c : fAssigned)
    if (c.first.Track()->Length() > minLength)
      n++;
  return n;
}
...
pma::VtxCandidate::Add(const pma::TrkCandidate& trk)
{
  ...
  if (fAssigned.size() > 2) {
    ...
    for (size_t m = 0; m < p0->Nodes().size() - 1; m++) {
      ...
      for (size_t n = 0; n < trk.Track()->Nodes().size() - 1; n++) {
        pma::Segment3D* seg_n =
          trk.Track()->NextSegment(trk.Track()->Nodes()[n]);
        ln = seg_n->Length();
        if (ln < fSegMinLength)
          continue;
        ...
        if (d < d_best) {
          ...
          if (lm + ln >
              0.8 * d_dist * l_best) // take closer if not much shorter
          {
            min_mse = mse;
            n_best = n;
            m_best = m;
...
pma::VtxCandidate::MergeWith(const pma::VtxCandidate& other)
{
  ...
  if (ntrk) {
    ...
    // std::cout << "try: " << d << " mse0:" << sqrt(mse0) << " mse1:" <<
    // sqrt(mse1) << std::endl;

    double mse = Compute();
    mf::LogVerbatim("pma::VtxCandidate")
      << "out: " << Size() << " mse:" << sqrt(mse) << " dw:" << dw;
    // std::cout << "out: " << Size() << " mse:" << sqrt(mse) << " dw:" << dw <<
    // std::endl;
...
pma::VtxCandidate::Compute()
{
  ...
  for (size_t s = 0; s < segments.size(); s++) {
    ...
    pproj =
      pma::GetProjectionToSegment(result, vprev->Point3D(), vnext->Point3D());
...
bool
pma::VtxCandidate::JoinTracks(pma::TrkCandidateColl& tracks,
                              pma::TrkCandidateColl& src)
{
  if (tracksJoined) {
    mf::LogError("pma::VtxCandidate")
  ...
  for (size_t i = 0; i < fAssigned.size(); i++) {
    ...
    if ((idx == 0) && (f * ds <= kMinDistToNode)) {
      if (i == 0) {
        ...
        if (trk->AttachTo(vtxCenter))
          nOK++;
      }
    } else if ((idx + 2 == trk->Nodes().size()) &&
      ...
      if (hasInnerCenter || !canFlipPrev) {
        ...
        pma::Track3D* t0 =
          trk->Split(idx); // makes both tracks attached to each other
  ...
  if (vtxCenter) {
    ...
    if (vtxCenter->NextCount())
      ...
      throw cet::exception("pma::VtxCandidate")
        << "Vertex with no segments attached.";

    pma::Track3D* rootTrk = rootSeg->Parent()->GetRoot();
    if (!rootTrk)
      rootTrk = rootSeg->Parent();

    std::vector<pma::Track3D const*>
      branchesToRemove; // change to a more simple check
