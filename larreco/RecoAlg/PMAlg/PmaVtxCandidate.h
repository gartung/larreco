...
public:
  static constexpr double kMaxDistToTrack{
    4.0}; // max. dist. track to center to create vtx
  static constexpr double kMinDistToNode{
    2.0}; // min. dist. to node needed to split segment
  ...
  std::pair<pma::Track3D*, size_t>
  Track(size_t i) const
  {
    return std::pair<pma::Track3D*, size_t>(fAssigned[i].first.Track(),
                                            fAssigned[i].second);
  }

private:
  bool
  has(const std::vector<int>& v, int id) const
  {
    for (auto c : v)
      if (c == id)
        return true;
