...
{
  if (vstart->TPC() == vstop->TPC())
    fTPC = vstart->TPC();
  if (vstart->Cryo() == vstop->Cryo())
    fCryo = vstart->Cryo();
}
...
pma::Segment3D::SumDist2Hits() const
{
  ...
  for (auto h : fAssignedHits) {
    if (h->IsEnabled()) {
      ...
      sum +=
        OptFactor(view) *
        h->GetSigmaFactor() // alpha_i * (hit_amp / hit_max_amp)
        *
...
double
pma::Segment3D::GetDist2(const TVector3& psrc,
                         const TVector3& p0,
                         const TVector3& p1)
{
  ...
  if (v1Norm2 >= 1.0E-6) // >= 0.01mm
  {
    ...
    if ((v0v1 > 0.0) && (v2v1 < 0.0)) {
      ...
      if (mag01_square != 0.0)
        cosine01_square = v0v1 * v0v1 / mag01_square;

      result = (1.0 - cosine01_square) * v0Norm2;
    } else // increase distance to prefer hit assigned to the vertex, not
           // segment
    {
      if (v0v1 <= 0.0)
...
double
pma::Segment3D::GetDist2(const TVector2& psrc,
                         const TVector2& p0,
                         const TVector2& p1)
{
