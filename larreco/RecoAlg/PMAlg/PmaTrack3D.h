 ...
 * 2D projections. Based on the "Precise 3D track reco..." AHEP (2013) 260820,
 * with all the tricks that we developed later and with the work for the
 * track-vertex topology optimization done at FNAL.
 *
 *          Progress:
 *             May-June 2015:  basic functionality for single (not-branching) 3D
 * track optimization and dQ/dx. August 2015:    3D vertex finding and branching
 * tracks optimization.
 */
...
public:
  ...
  double Dist2(const TVector2& p2d,
               unsigned int view,
               unsigned int tpc,
  ...
  void AutoFlip(pma::Track3D::EDirection dir,
                double thr = 0.0,
                unsigned int n = 0);
  ...
  /// Count close 2D hits.
  unsigned int TestHits(
    const std::vector<art::Ptr<recob::Hit>>& hits,
    double dist = 0.4) const; // max acceptable distance [cm]

  int NextHit(int index,
              unsigned int view = geo::kZ,
              bool inclDisabled = false) const;
  int PrevHit(int index,
              unsigned int view = geo::kZ,
              bool inclDisabled = false) const;

  /// Length of the track part associated with index'th hit. Calculated as a
  /// half distance to the preceding hit plus half distance to the subsequent
  /// hit. In case of the first (last) hit - missing part is estimated as 1/4 of
  /// the distance to the next (previous) hit. NOTE: only hits from a given view
  /// are considered; other hits are accounted for segment lengths but overall
  /// dx is calculated between hits in given view.
  double HitDxByView(size_t index, unsigned int view) const;

  /// Sequence of <hit_index, (wire, drift, X, Y, Z, dE, dx, range)> values for
  /// the track, hits tagged as outliers are skipped by default.
  /** Results are pushed into the dedx vector given in the function arguments:
      ...
      Parameters:
        ...
        skip  - number of hits to skip at the begining (first hit has poorly
     estimated segment length so it can be convenient to set skip=1 and handle
     first hit charge manually); inclDisabled - if true then artificial hits
     added with CompleteMissingWires() are used, otherwise only true hits found
     in ADC are used.
  ...
  std::vector<float> DriftsOfWireIntersection(unsigned int wire,
  ...
  void InsertNode(TVector3 const& p3d,
                  size_t at_idx,
                  unsigned int tpc,
                  unsigned int cryo);
  ...
  pma::Track3D* GetRoot();
  bool GetBranches(std::vector<pma::Track3D const*>& branches,
...
private:
  ...
  bool GetUnconstrainedProj3D(art::Ptr<recob::Hit> hit,
                              TVector3& p3d,
                              double& dist2) const;
