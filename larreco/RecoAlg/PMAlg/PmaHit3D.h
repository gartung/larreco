 ...
 *
 *          Hit 3D wrapped around recob::Hit. Adds support for PMA
 * optimizations. See PmaTrack3D.h file for details.
 */
...
private:
  ...
  TVector2 fProjection2D; // projection to polygonal line in 2D wire view,
                          // scaled to [cm]
  float fSegFraction;     // segment fraction set by the projection
  float fSigmaFactor;     // impact factor on the objective function

  double fDx; // dx seen by corresponding 2D hit, set during dQ/dx sequece
              // calculation
