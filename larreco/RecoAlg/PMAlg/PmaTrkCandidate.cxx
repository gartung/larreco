 ...
 *
 *          Candidate for 3D track. Used to test 2D cluster associations,
 * validadion result, MSE value. See PmaTrack3D.h file for details.
 */
...
pma::TrkCandidateColl::getCandidateIndex(pma::Track3D const* candidate) const
{
  for (size_t t = 0; t < fCandidates.size(); ++t)
    if (fCandidates[t].Track() == candidate)
      return t;
  return -1;
}
...
pma::TrkCandidateColl::setParentDaughterConnections()
{
  ...
  for (t = 0; t < fCandidates.size(); ++t) {
    ...
    {
      pma::Track3D const* parentTrk =
        static_cast<pma::Segment3D*>(firstNode->Prev())->Parent();
      fCandidates[t].SetParent(getCandidateIndex(parentTrk));
    } else if (fCandidates[t].Parent() <
               0) // parent not reconstructed and not yet set, add empty
    ...
    for (size_t n = 1; n < trk->Nodes().size(); ++n) {
      ...
      for (size_t i = 0; i < node->NextCount(); ++i) {
        pma::Track3D const* daughterTrk =
          static_cast<pma::Segment3D*>(node->Next(i))->Parent();
        if (daughterTrk != trk) {
          int idx = getCandidateIndex(daughterTrk);
          if (idx >= 0)
            fCandidates[t].Daughters().push_back((size_t)idx);
        }
      }
    }
  }
    ...
    if (tEntry.first >= 0) {
      ...
      while (!tEntry.second.empty()) {
        ...
        if (pFront[coordinate] > pBack[coordinate]) {
          if (setTreeOriginAtBack(trk)) {
            break;
          } else {
            mf::LogWarning("pma::TrkCandidateColl")
        ...
        if (attempts++ > 2)
          break; // do not try all the tracks in the queue...
      }
    }
}
// ------------------------------------------------------
...
pma::TrkCandidateColl::setTreeOriginAtFront(pma::Track3D* trk)
{
  ...
  if (n->Prev()) {
    ...
    if (incoming->NextSegment(n)) // upfff, need to break the parent track
    {
      ...
      if (idx >= 0) {
        pma::Track3D* u =
          incoming->Split(idx, false); // u is in front of incoming
        if (u) {
...
pma::TrkCandidateColl::flipTreesByDQdx()
{
  ...
  for (auto const& t : fCandidates) {
    if (t.IsValid())
      trkMap[t.TreeId()].push_back(t.Track());
  }

  for (auto& tEntry : trkMap) {
    std::sort(
      tEntry.second.begin(), tEntry.second.end(), pma::bTrack3DLonger());
...
{
  fCandidates[idx1].Track()->ExtendWith(
    fCandidates[idx2].Track()); // deletes track at idx2
...
pma::Track3D*
pma::TrkCandidateColl::getTreeCopy(pma::TrkCandidateColl& dst,
                                   size_t trkIdx,
                                   bool isRoot)
{
  pma::Track3D* trk = fCandidates[trkIdx].Track();
  ...
  if (!isRoot) {
    ...
    if (segThisCopy)
      vtxCopy = static_cast<pma::Node3D*>(segThisCopy->Next());
  }
