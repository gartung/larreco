 ...
 *
 *          Build 3D segments and whole tracks by simultaneous matching hits in
 * 2D projections. See PmaTrack3D.h file for details.
 */
  ...
  fTag(src.fTag)
{
  ...
  for (auto const& node : src.fNodes)
    fNodes.push_back(new pma::Node3D(node->Point3D(),
                                     node->TPC(),
                                     node->Cryo(),
                                     node->IsVertex(),
                                     node->GetDriftShift()));
...
pma::Track3D::~Track3D()
{
  ...
  for (size_t i = 0; i < fNodes.size(); i++)
    if (!fNodes[i]->NextCount() && !fNodes[i]->Prev())
      delete fNodes[i];
}

bool
pma::Track3D::Initialize(float initEndSegW)
{
  if (!HasTwoViews(2)) {
  ...
  // single tpc, many tpc's are ok, but need to be handled from
  // ProjectionMatchingAlg::buildMultiTPCTrack()
  int tpc = tpcs.front();

  if (InitFromRefPoints(tpc, cryo))
    mf::LogVerbatim("pma::Track3D")
      << "Track initialized with 3D reference points.";
  else {
    if (InitFromHits(tpc, cryo, initEndSegW))
      mf::LogVerbatim("pma::Track3D")
        << "Track initialized with hit positions.";
    else {
      InitFromMiddle(tpc, cryo);
      mf::LogVerbatim("pma::Track3D")
...
bool
pma::Track3D::InitFromHits(int tpc, int cryo, float initEndSegW)
{
  ...
  if (hit0_a && hit0_b && hit1_a && hit1_b) {
    ...
    x = 0.5 * (detprop->ConvertTicksToX(
                 hit1_a->PeakTime(), hit1_a->View2D(), tpc, cryo) +
               detprop->ConvertTicksToX(
                 hit1_b->PeakTime(), hit1_b->View2D(), tpc, cryo));
    geom->IntersectionPoint(hit1_a->Wire(),
                            hit1_b->Wire(),
                            hit1_a->View2D(),
                            hit1_b->View2D(),
                            cryo,
  ...
  if (pma::Dist2(fNodes.front()->Point3D(), fNodes.back()->Point3D()) <
      (0.3 * 0.3)) {
...
bool
pma::Track3D::InitFromRefPoints(int tpc, int cryo)
{
  if (fAssignedPoints.size() < 2)
    return false;
  ...
  v2 *= -scale;
  v2 += mean;
  std::sort(fAssignedPoints.begin(),
            fAssignedPoints.end(),
            pma::bSegmentProjLess(v1, v2));
  for (size_t i = 0; i < fAssignedPoints.size(); i++) {
...
void
pma::Track3D::InitFromMiddle(int tpc, int cryo)
{
  art::ServiceHandle<geo::Geometry const> geom;
...
bool
pma::Track3D::push_back(const art::Ptr<recob::Hit>& hit)
{
...
pma::Track3D::GetDirection3D(size_t index) const
{
  ...
  if (pe) {
    ...
    throw cet::exception("pma::Track3D")
      << "GetDirection3D(): direction of a not assigned hit " << index
      << " (size: " << fHits.size() << ")" << std::endl;
  }
}

void
pma::Track3D::AddHits(const std::vector<art::Ptr<recob::Hit>>& hits)
{
...
pma::Track3D::Cryos() const
{
  ...
  for (size_t i = 0; i < size(); i++) {
    ...
    if (!found)
      cryo_idxs.push_back(cryo);
  }
  return cryo_idxs;
}

std::pair<TVector2, TVector2>
pma::Track3D::WireDriftRange(unsigned int view,
                             unsigned int tpc,
...
bool
pma::Track3D::Flip(std::vector<pma::Track3D*>& allTracks)
{
  if (fNodes.size() < 2) {
    return true;
  }
  ...
  if (n->Prev()) {
    ...
    if (t->NextSegment(n)) // starts from middle of another track: need to break
                           // that one first
    {
      ...
      if (idx >= 0) {
        ...
        if (u) {
          ...
          if (u->Flip(allTracks)) {
            ...
            mf::LogWarning("pma::Track3D")
              << "Flip(): Could not flip after split (but new track is "
                 "preserved!!).";
            return false;
          }
        } else {
...
pma::Track3D::AutoFlip(pma::Track3D::EDirection dir, double thr, unsigned int n)
{
  ...
  if (dedxInViews[0].size() > 2 * dedxInViews[2].size())
    bestView = 0;
  if (dedxInViews[1].size() > 2 * dedxInViews[2].size())
    bestView = 1;
  ...
  if (dedx.size() > 4) {
    ...
    if (dedx.front()[2] > 0.0F)
      dEdxStart = dedx.front()[5] / dedx.front()[6];
    if (dedx.back()[2] > 0.0F)
      dEdxStop = dedx.back()[5] / dedx.back()[6];
  } else
    return;
  ...
  if ((dir == pma::Track3D::kBackward) &&
      (dEdxStop > (1.0 + thr) * dEdxStart)) {
    mf::LogVerbatim("pma::Track3D")
...
bool
pma::Track3D::AutoFlip(std::vector<pma::Track3D*>& allTracks,
...
double
pma::Track3D::TestHitsMse(const std::vector<art::Ptr<recob::Hit>>& hits,
                          bool normalized) const
{
  ...
  for (auto const& h : hits) {
    ...
    mse +=
      Dist2(pma::WireDriftToCm(wire, drift, view, tpc, cryo), view, tpc, cryo);
  }
  if (normalized)
...
unsigned int
pma::Track3D::TestHits(const std::vector<art::Ptr<recob::Hit>>& hits,
                       double dist) const
{
  if (hits.empty()) {
                                 ...
                                 bool inclDisabled) const
{
  ...
  if (j >= size())
    return 0.0F;     // no charged hits at all
  ...
  while (j <= jmax) {
    ...
    while (((m < step) || (dR < 0.1) || (dr == 0.0)) && (j <= jmax)) {
      ...
      if (j > jmax)
        break; // no more hits in this view
...
std::vector<float>
pma::Track3D::DriftsOfWireIntersection(unsigned int wire,
                                       unsigned int view) const
{
  std::vector<float> drifts;
  for (size_t i = 0; i < fNodes.size() - 1; i++) {
    int tpc = fNodes[i]->TPC(), cryo = fNodes[i]->Cryo();
    if ((tpc != fNodes[i + 1]->TPC()) || (cryo != fNodes[i + 1]->Cryo()))
      continue;

    TVector2 p0 = pma::CmToWireDrift(fNodes[i]->Projection2D(view).X(),
                                     fNodes[i]->Projection2D(view).Y(),
                                     view,
                                     fNodes[i]->TPC(),
                                     fNodes[i]->Cryo());
    TVector2 p1 = pma::CmToWireDrift(fNodes[i + 1]->Projection2D(view).X(),
...
pma::Track3D::AddNode()
{
  ...
  for (unsigned int i = si + 1; i < fNodes.size(); i++) {
    ...
    if (segHits < 15) {
      if ((nHitsByView[0] == 0) &&
          ((nHitsByView[1] < 4) || (nHitsByView[2] < 4)))
        continue;
      if ((nHitsByView[1] == 0) &&
          ((nHitsByView[0] < 4) || (nHitsByView[2] < 4)))
        continue;
      if ((nHitsByView[2] == 0) &&
          ((nHitsByView[0] < 4) || (nHitsByView[1] < 4)))
        continue;
    }

    nHits = fNodes[i]->NEnabledHits() + seg->NEnabledHits() +
            fNodes[i - 1]->NEnabledHits();
  ...
  if (maxSegHits > 1) {
    ...
    if ((nHitsByView[2] >= nHitsByView[1]) &&
        (nHitsByView[1] >= nHitsByView[0])) {
      maxViewIdx = 2;
      midViewIdx = 1;
    } else if ((nHitsByView[1] >= nHitsByView[2]) &&
               (nHitsByView[2] >= nHitsByView[0])) {
      maxViewIdx = 1;
      midViewIdx = 2;
    } else if ((nHitsByView[0] >= nHitsByView[2]) &&
               (nHitsByView[2] >= nHitsByView[1])) {
      maxViewIdx = 0;
      midViewIdx = 2;
    } else if ((nHitsByView[2] >= nHitsByView[0]) &&
               (nHitsByView[0] >= nHitsByView[1])) {
      maxViewIdx = 2;
      midViewIdx = 0;
    } else if ((nHitsByView[0] >= nHitsByView[1]) &&
               (nHitsByView[1] >= nHitsByView[2])) {
      maxViewIdx = 0;
      midViewIdx = 1;
    } else if ((nHitsByView[1] >= nHitsByView[0]) &&
               (nHitsByView[0] >= nHitsByView[2])) {
      maxViewIdx = 1;
      midViewIdx = 0;
    }
    if (nHitsByView[midViewIdx] < 2)
      midViewIdx = maxViewIdx;
    ...
    i0 = i;
    i++;
    while ((i < maxSeg->NHits()) && !((maxSeg->Hit(i).View2D() == midViewIdx) &&
                                      maxSeg->Hit(i).IsEnabled())) {
      i++;
    }
    i1 = i;
...
void
pma::Track3D::InsertNode(TVector3 const& p3d,
                         size_t at_idx,
                         unsigned int tpc,
                         unsigned int cryo)
{
...
pma::Track3D*
pma::Track3D::Split(size_t idx, bool try_start_at_idx)
{
  ...
  n = fNodes.front();
  t0->fNodes.push_back(new pma::Node3D(
    n->Point3D(), n->TPC(), n->Cryo(), false, n->GetDriftShift()));
  t0->RebuildSegments();
  RebuildSegments();
...
pma::Track3D::AttachToOtherTPC(pma::Node3D* vStart)
{
  ...
  fSegments.insert(fSegments.begin(),
                   new pma::Segment3D(this, fNodes[0], fNodes[1]));
...
bool
pma::Track3D::GetBranches(std::vector<pma::Track3D const*>& branches,
                          bool skipFirst) const
{
  ...
  for (size_t i = i0; i < fNodes.size(); ++i)
    for (size_t n = 0; n < fNodes[i]->NextCount(); n++) {
      ...
      if (seg && (seg->Parent() != this))
        seg->Parent()->GetBranches(branches, true);
    }
...
pma::Track3D::IsAttachedTo(pma::Track3D const* trk) const
{
  ...
  if (!trk->GetBranches(branchesTrk))
    return true; // has loop - better check the reason
  if (!GetBranches(branchesThis))
    return true; // has loop - better check the reason
...
double
pma::Track3D::Optimize(int nNodes,
                       double eps,
                       bool selAllHits,
                       bool setAllNodes,
                       size_t selSegHits,
                       size_t selVtxHits)
{
  ...
  do {
    ...
    switch (nNodes) {
      case 0:
        stop = true;
        break; // just optimize existing vertices
...
bool
pma::Track3D::UpdateParamsInTree(bool skipFirst)
{
  ...
  while (vtx) {
    ...
    for (size_t i = 0; i < vtx->NextCount(); i++) {
      ...
      if (seg != segThis) {
        isOK &= seg->Parent()->UpdateParamsInTree(true);
      }
    }
...
pma::Track3D*
pma::Track3D::GetNearestTrkInTree(const TVector3& p3d_cm,
                                  double& dist,
                                  bool skipFirst)
{
  pma::Node3D* vtx = fNodes.front();
...
pma::Track3D::ReassignHitsInTree(pma::Track3D* trkRoot)
{
  ...
  while (HasTwoViews(2) && (i < size())) {
    ...
    nearestTrk = GetNearestTrkInTree(
      hit->Point2D(), hit->View2D(), hit->TPC(), hit->Cryo(), dmin);
...
void
pma::Track3D::SetT0FromDx(double dx)
{
  ...
  const geo::TPCGeo& tpcGeo =
    geom->TPC(fNodes.front()->TPC(), fNodes.front()->Cryo());
...
{
  if (fSegments.front()->TPC() < 0)
    skipFrontVtx = false;
  if (fSegments.back()->TPC() < 0)
    skipBackVtx = false;
...
bool
pma::Track3D::GetUnconstrainedProj3D(art::Ptr<recob::Hit> hit,
                                     TVector3& p3d,
                                     double& dist2) const
{
  TVector2 p2d = pma::WireDriftToCm(hit->WireID().Wire,
                                    hit->PeakTime(),
                                    hit->WireID().Plane,
                                    hit->WireID().TPC,
                                    hit->WireID().Cryostat);
  ...
  if (seg) {
    ...
    return prev->SameTPC(
      p3d); // 3D can be beyond the segment endpoints => in other TPC
  }
  ...
  do {
    ...
    for (size_t i = 0; i < vtx->NHits(); i++) {
      ...
      if (vtx->Hit(i).IsEnabled())
        hasHits[vtx->Hit(i).View2D()] = 1;
      if (nextHit && nextHit->IsEnabled())
        hasHits[nextHit->View2D()] = 1;
...
pma::Track3D::SelectHits(float fraction)
{
  ...
  if (fraction > 1.0F)
    fraction = 1.0F;
  if (fraction < 1.0F)
    std::sort(fHits.begin(), fHits.end(), pma::bTrajectory3DDistLess());
...
pma::Track3D::MakeProjection()
{
  ...
  if (!(fNodes.front()->IsFrozen()) && (fNodes.back()->NextCount() == 0)) {
    skipBackVtx = true;
  }

  for (auto h : fHits) // assign hits to nodes/segments
  {
    pe = GetNearestElement(
      h->Point2D(), h->View2D(), h->TPC(), skipFrontVtx, skipBackVtx);
    pe->AddHit(h);
  }
...
pma::Track3D::UpdateParams()
{
  ...
  if (avgDist2Root > 0) {
    fPenaltyValue = fPenaltyFactor * pow((double)fSegments.size(), 1.8) *
                    avgDist2Root / (fHitsRadius * nCubeRoot);

    fSegStopValue =
      (int)(fSegStopFactor * nCubeRoot * fHitsRadius / avgDist2Root);
    if (fSegStopValue < fMinSegStop)
      fSegStopValue = fMinSegStop;
    return true;
  } else {
    fPenaltyValue = 1;
    fSegStopValue = 1;
...
pma::Track3D::CheckEndSegment(pma::Track3D::ETrackEnd endCode)
{
  ...
  switch (endCode) {
    case pma::Track3D::kBegin:
      if (fSegments.front()->IsFrozen())
        return false;
      if (fNodes.front()->NextCount() > 1)
        return false;
      v1 = 0;
      v2 = 1;
      break;
    case pma::Track3D::kEnd:
      if (fSegments.back()->IsFrozen())
        return false;
      if (fNodes.back()->NextCount() > 1)
        return false;
      v1 = fNodes.size() - 1;
      v2 = fNodes.size() - 2;
      break;
  ...
  if (fNodes[v1]->TPC() != fNodes[v2]->TPC())
    return false;

  double g1, g0 = GetObjFunction();

  if (SwapVertices(v1, v2))
    RebuildSegments();
  MakeProjection();
  g1 = GetObjFunction();
  ...
  for (size_t i = 0; i < size(); i++) {
    ...
    switch (hit->View2D()) {
      case geo::kZ:
        ...
        break;
      case geo::kV:
        hitsInd2.push_back(hit);
        break;
      case geo::kU:
        hitsInd1.push_back(hit);
        break;
    }
  }
