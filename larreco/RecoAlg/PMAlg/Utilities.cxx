...
pma::GetHitsCount(const std::vector<pma::Hit3D*>& hits, unsigned int view)
{
  ...
  return ranges::count_if(hits,
                          [view](auto hit) { return view == hit->View2D(); });
}
...
pma::GetSummedADC(const std::vector<pma::Hit3D*>& hits, unsigned int view)
{
  ...
  if (view == geo::kUnknown) {
    return accumulate(hits | view::transform(to_summed_adc), 0.);
  }
  return accumulate(
    hits | view::filter([view](auto hit) { return view == hit->View2D(); }) |
...
pma::GetSummedAmpl(const std::vector<pma::Hit3D*>& hits, unsigned int view)
{
  ...
  if (view == geo::kUnknown) {
    return accumulate(hits | view::transform(to_amplitude), 0.);
  }
  return accumulate(
    hits | view::filter([view](auto hit) { return view == hit->View2D(); }) |
...
pma::GetHitsRadius2D(const std::vector<pma::Hit3D*>& hits, bool exact)
{
  ...
  auto const mean_point =
    accumulate(hits | view::transform(to_2d_point), TVector2{}) *
    (1. / hits.size());
...
double
pma::GetSegmentProjVector(const TVector2& p,
                          const TVector2& p0,
                          const TVector2& p1)
{
...
double
pma::GetSegmentProjVector(const pma::Vector2D& p,
...
double
pma::GetSegmentProjVector(const TVector3& p,
                          const TVector3& p0,
                          const TVector3& p1)
{
...
double
pma::GetSegmentProjVector(const pma::Vector3D& p,
...
TVector2
pma::GetProjectionToSegment(const TVector2& p,
                            const TVector2& p0,
                            const TVector2& p1)
{
  TVector2 const v1(p1 - p0);
...
TVector3
pma::GetProjectionToSegment(const TVector3& p,
                            const TVector3& p0,
                            const TVector3& p1)
{
  TVector3 const v1(p1 - p0);
...
pma::SolveLeastSquares3D(
  const std::vector<std::pair<TVector3, TVector3>>& lines,
  TVector3& result)
{
  ...
  for (size_t r = 0; r < 3; r++) {
    ...
    for (size_t c = 0; c < 3; c++) {
      ...
      for (size_t v = 0; v < P.size(); v++) {
        // w[1] = fWeights[v]; // to remember that individual coordinates can be
        // supressed... w[2] = fWeights[v];
  ...
  double mse = 0.0;
  for (size_t v = 0; v < lines.size(); v++) {
    TVector3 const pproj =
      pma::GetProjectionToSegment(result, lines[v].first, lines[v].second);

    double const dx =
      result.X() - pproj.X(); // dx, dy, dz and the result point can be weighted
    double const dy =
      result.Y() - pproj.Y(); // here (linearly) by each line uncertainty
...
bool
pma::bTrack3DLonger::operator()(const pma::TrkCandidate& t1,
...
{
  if (s0 == s1)
    mf::LogError("pma::bSegmentProjLess") << "Vectors equal!";
}
