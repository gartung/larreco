  ...
  , fIsVertex(vtx)
{
  ...
  fMaxX = detprop->ConvertTicksToX(
    detprop->NumberTimeSamples() - 1, lastPlane, tpc, cryo);
  //    fMinX = fTpcGeo.MinX();
  //    fMaxX = fTpcGeo.MaxX();
  if (fMaxX < fMinX) {
    double tmp = fMaxX;
...
pma::Node3D::UpdateProj2D()
{
  for (size_t i = 0; i < fTpcGeo.Nplanes(); ++i) {
    fProj2D[i].Set(fTpcGeo.Plane(i).PlaneCoordinate(fPoint3D),
...
pma::Node3D::GetDirection3D() const
{
  pma::Element3D* seg = 0;
  if (next) {
    seg = dynamic_cast<pma::Element3D*>(next);
  } else if (prev) {
...
pma::Node3D::SetProjection(pma::Hit3D& h) const
{
  ...
  if (prev && next) {
    ...
    // hit on the previous segment side, sorting on the -cosine(prev_seg, point)
    // /max.val. = 1/
    if (cosineN <= cosine)
      h.SetProjection(p, -(float)cosine);
    // hit on the next segment side, sorting on the 1+cosine(next_seg, point)
    // /min.val. = 1/
    else
...
pma::Node3D::Length2() const
{
  double l = 0.0;
  if (next)
    l += (static_cast<pma::Segment3D*>(next))->Length();
  if (prev)
    l += (static_cast<pma::Segment3D*>(prev))->Length();
...
pma::Node3D::SegmentCos() const
{
  if (prev && next) {
    ...
    pma::Vector3D v1(vStop1.X() - fPoint3D.X(),
                     vStop1.Y() - fPoint3D.Y(),
                     vStop1.Z() - fPoint3D.Z());
    pma::Vector3D v2(vStop2.X() - fPoint3D.X(),
                     vStop2.Y() - fPoint3D.Y(),
                     vStop2.Z() - fPoint3D.Z());
    double mag = sqrt(v1.Mag2() * v2.Mag2());
    double cosine = 0.0;
    if (mag != 0.0)
      cosine = v1.Dot(v2) / mag;
    return cosine;
  } else {
    mf::LogError("pma::Node3D")
...
// *** Note: should be changed / generalized for horizontal wire planes (e.g.
// 2-phase LAr). ***
double
...
pma::Node3D::PiInWirePlane() const
{
  if (prev && NextCount()) {
    ...
    unsigned int nInd1 =
      NHits(geo::kU) + seg0->NHits(geo::kU) + seg1->NHits(geo::kU);

    if (fHitsRadius > 0.0F)
      return (1.0 + SegmentCosWirePlane()) * fHitsRadius * fHitsRadius /
             (4 * nInd1 + 1.0);
    else
...
// Constraint on two segments angle in projection to plane parallel to wire
// plane, suppressed by the orientation in the plane transverse to wire plane
// (only sections with low variation of drift time are penalized with this
// constraint); PiInWirePlane() components are reduced if there are Ind1 /
// geo::kU hits which add information to the object shape.
// *** Note: should be changed / generalized for horizontal wire planes (e.g.
// 2-phase LAr). ***
double
...
pma::Node3D::Pi(float endSegWeight, bool doAsymm) const
{
  ...
  if (prev && NextCount()) {
    ...
    if (nSeg == 1)
      pi_result = endSegWeight * seg->Length2();
    return pi_result;
  }
}
...
pma::Node3D::MakeGradient(float penaltyValue, float endSegWeight)
{
  ...
  // if (fQPenaltyFactor > 0.0F) gz += fQPenaltyFactor * QPenalty();
  // <----------------------- maybe later..
...
double
pma::Node3D::StepWithGradient(float alfa,
                              float tol,
                              float penalty,
                              float weight)
{
  ...
  while (fabs(t1 - t3) > tol) {
    ...
    if (fabs(t - t2) < 0.2 * tol)
      break; // start in a new direction
    if (g < g2) {
      if (t < t2) {
        t3 = t2;
        g3 = g2;
