...
pma::Node3D::Node3D(const Point3D_t& p3d, unsigned int tpc, unsigned int cryo, bool vtx) :
	fIsVertex(vtx)
{
	fTPC = tpc; fCryo = cryo;
...
bool pma::Node3D::SameTPC(const Point3D_t& p3d, float margin) const
{
...
bool pma::Node3D::SetPoint3D(const Point3D_t& p3d)
{
	fPoint3D = p3d;
...
void pma::Node3D::SetProjection(pma::Hit3D& h) const
{
	...
	else
	{
		...
		if (fFrozen) // limit 3D positions to outermost node if frozen
		{
			h.SetPoint3D(fPoint3D);
		}
...
double pma::Node3D::SegmentCos(void) const
{
	if (prev && next)
	{
		pma::Node3D const* vStop1 = static_cast< pma::Node3D const* >(prev->Prev());
		pma::Node3D const* vStop2 = static_cast< pma::Node3D const* >(next->Next());
		auto v1 = vStop1->fPoint3D - fPoint3D;
		auto v2 = vStop2->fPoint3D - fPoint3D;
		double const mag2 = v1.Mag2() * v2.Mag2();
		return (mag2 != 0.0)? v1.Dot(v2) / std::sqrt(mag2): 0.0;
	}
	else
	{
...
double pma::Node3D::SegmentCosWirePlane(void) const
{
	if (prev && next)
	{
		pma::Node3D const* vStop1 = static_cast< pma::Node3D const* >(prev->Prev());
		pma::Node3D const* vStop2 = static_cast< pma::Node3D const* >(next->Next());
		Vector2D_t v1(vStop1->fPoint3D.Y() - fPoint3D.Y(), vStop1->fPoint3D.Z() - fPoint3D.Z());
		Vector2D_t v2(vStop2->fPoint3D.Y() - fPoint3D.Y(), vStop2->fPoint3D.Z() - fPoint3D.Z());
		double const mag2 = v1.Mag2() * v2.Mag2();
		return (mag2 != 0.0)? v1.Dot(v2) / std::sqrt(mag2): 0.0;
	}
	else
...
double pma::Node3D::SegmentCosTransverse(void) const
{
	if (prev && next)
	{
		pma::Node3D const* vStop1 = static_cast< pma::Node3D const* >(prev->Prev());
		pma::Node3D const* vStop2 = static_cast< pma::Node3D const* >(next->Next());
		Vector2D_t v1( vStop1->fPoint3D.X() - fPoint3D.X(),
		               vStop1->fPoint3D.Z() - fPoint3D.Z()),
		           v2( vStop2->fPoint3D.X() - fPoint3D.X(),
...
double pma::Node3D::EndPtCos2Transverse(void) const
{
	if (prev && next)
	{
		pma::Node3D const* vStart = static_cast< pma::Node3D const* >(prev->Prev());
		pma::Node3D const* vStop = static_cast< pma::Node3D const* >(next->Next());

		double dx = vStop->Point3D().X() - vStart->Point3D().X();
...
{	
	...
	Point3D_t tmp = fPoint3D, gpoint = fPoint3D;
...
double pma::Node3D::StepWithGradient(float alfa, float tol, float penalty, float weight)
{
	...
	do
	{
		...
		if (!SetPoint3D(makePoint3D(gpoint))) // stepped out of allowed volume
		{
			...
			else { SetPoint3D(makePoint3D(tmp)); return 0.0; } // exit with restored original position
		}
		...
		if (++steps > 1000) { SetPoint3D(makePoint3D(tmp)); return 0.0; }
	...
	if (steps == 1)
	{
		...
		do
		{
			...
			//if (t2 == t1) { SetPoint3D(makePoint3D(tmp)); return 0.0F; }

			// break: starting point is very close to the minimum
			if (fabs(t2 - t1) < tol) { SetPoint3D(makePoint3D(tmp)); return 0.0; }

			gpoint = tmp;
			gpoint += (makeTVector3(fGradient) * t2);
			if (!SetPoint3D(makePoint3D(gpoint))) // select the best point to exit
			{
				...
				else { SetPoint3D(makePoint3D(tmp)); return 0.0; }
			}
			g2 = GetObjFunction(penalty, weight);
