...
//
//      Build 3D segments and whole tracks by matching an object 2D projections
//      to hits, simultaneously in multiple wire planes. Based on the algorithm
//      first presented in "Precise 3D track reco..." AHEP (2013) 260820, with
//      all the tricks that we developed later and with the work for the
//      full-event topology optimization that is still under construction now
//      (2015).
//
//      The algorithm class provides functionality to build a track from
//      selected hits. These can be detailed tracks or just simple segments (if
//      the number of nodes to add is set to 0). The parameters of optimization
//      algorithm, fixed nodes and 3D reference points can be configured here.
//      Please, check the track making module to find a way of selecting
//      appropriate clusteres:
...
public:
  struct Config {
    ...
    fhicl::Atom<double> OptimizationEps{
      ...
      Comment("relative change of the obj.fn which stops optimization after "
              "adding a node")};

    fhicl::Atom<double> FineTuningEps{
      Name("FineTuningEps"),
      Comment("relative change of the obj.fn which stops fine-tuning of "
              "optimized track")};
    ...
    fhicl::Atom<double> HitTestingDist2D{
      ...
      Comment("max. distance [cm] used in testing compatibility of hits with "
              "the track")};

    fhicl::Atom<double> MinTwoViewFraction{
      Name("MinTwoViewFraction"),
      Comment("min. fraction of track length covered with hits from many 2D "
              "views intertwinted with each other")};

    fhicl::Atom<double> NodeMargin3D{
      Name("NodeMargin3D"),
      Comment("margin in [cm] around TPC for allowed track node positions")};

    fhicl::Atom<double> HitWeightU{Name("HitWeightU"),
                                   Comment("weights used for hits in U plane")};

    fhicl::Atom<double> HitWeightV{Name("HitWeightV"),
                                   Comment("weights used for hits in V plane")};

    fhicl::Atom<double> HitWeightZ{Name("HitWeightZ"),
  ...
  /// Calculate the fraction of trajectory seen by two 2D projections at least;
  /// even a prfect track starts/stops with the hit from one 2D view, then hits
  /// from other views come, which results with the fraction value high, but
  /// always < 1.0; wrong cluster matchings or incomplete tracks give
  /// significantly lower values.
  double twoViewFraction(pma::Track3D& trk) const;

  /// Count the number of hits that are closer than eps * fHitTestingDist2D to
  /// the track 2D projection.
  unsigned int
  testHits(const pma::Track3D& trk,
           const std::vector<art::Ptr<recob::Hit>>& hits,
  ...
  /// Test if hits at the track endpoinds do not stick out of TPC which they
  /// belong to. Here one can implement some configurable margin if needed for
  /// real data imeprfections.
  bool
  ...
  pma::Track3D* buildMultiTPCTrack(
    const std::vector<art::Ptr<recob::Hit>>& hits) const;

  /// Build a shower segment from sets of hits and attached to the provided
  /// vertex.
  pma::Track3D* buildShowerSeg(const std::vector<art::Ptr<recob::Hit>>& hits,
  ...
  pma::Track3D* buildSegment(const std::vector<art::Ptr<recob::Hit>>& hits,
                             const TVector3& point) const;
  ...
  pma::Track3D* extendTrack(const pma::Track3D& trk,
                            const std::vector<art::Ptr<recob::Hit>>& hits,
  ...
  std::vector<pma::Hit3D*> trimTrackToVolume(pma::Track3D& trk,
                                             TVector3 p0,
                                             TVector3 p1) const;
  ...
  /// compares dQ/dx of n hits at each end of the track (default is based on the
  /// track length).
  void
