...
bool trkf::TrackKalmanFitter::fitTrack(const recob::TrackTrajectory& traj, const int tkID, const SMatrixSym55& covVtx, const SMatrixSym55& covEnd,
  ...
  if (tryBothDirs_) {
    ...
    bool okfwd = fitTrack(position, direction, fwdcov, hits, traj.Flags(), tkID, pval, pdgid, fwdTrack, fwdHits, fwdoptionals);

    recob::Track bwdTrack;
    std::vector<art::Ptr<recob::Hit> > bwdHits;
    trkmkr::OptionalOutputs bwdoptionals;
    SMatrixSym55 bwdcov = covEnd;
    bool okbwd = fitTrack(position, -direction, bwdcov, hits, traj.Flags(), tkID, pval, pdgid, bwdTrack, bwdHits, bwdoptionals);

    if (okfwd==false && okbwd==false) {
      return false;
    } else if (okfwd==true && okbwd==true) {
      if ((fwdTrack.CountValidPoints()/(fwdTrack.Length()*fwdTrack.Chi2PerNdof()))>=(bwdTrack.CountValidPoints()/(bwdTrack.Length()*bwdTrack.Chi2PerNdof()))) {
	outTrack = fwdTrack;
	outHits = fwdHits;
...
bool trkf::TrackKalmanFitter::fitTrack(const Point_t& position, const Vector_t& direction, SMatrixSym55& trackStateCov, 
  ...
  bool fillok = fillResult(hits, tkID, pdgid, hitstatev, hitflagsv, fwdPrdTkState, fwdUpdTkState, hitstateidx, rejectedhsidx, sortedtksidx, outTrack, outHits, optionals);
  return fillok;
}
...
bool trkf::TrackKalmanFitter::doFitWork(KFTrackState& trackState, std::vector<HitState>& hitstatev, std::vector<recob::TrajectoryPointFlags::Mask_t>& hitflagsv,
  ...
  if (sortHitsByPlane_) {
	...
	for (auto h : p) {
	  std::cout << "hit #/Plane/Wire/x/mask: " << ch++ << " " << hitstatev[h].wireId().Plane << " " << hitstatev[h].wireId().Wire << " " << hitstatev[h].hitMeas() << " " << hitflagsv[h] << std::endl;
	}
      }
    }
	...
	for (unsigned int& ih = iterHitsInPlanes[iplane]; ih<hitsInPlanes[iplane].size(); ++ih) {
	  ...
	  if (dumpLevel_>1) std::cout << "ih=" << ih << " ihit=" << ihit << " size=" << hitsInPlanes[iplane].size() << std::endl;
	  ...
	  if (hitflags.isSet(recob::TrajectoryPointFlagTraits::NoPoint        ) ||
	    ...
	    if (dumpLevel_>1) std::cout << "rejecting hit flags=" << hitflags.isSet(recob::TrajectoryPointFlagTraits::NoPoint        ) << ", " << hitflags.isSet(recob::TrajectoryPointFlagTraits::ExcludedFromFit) << ", " << hitflags.isSet(recob::TrajectoryPointFlagTraits::Rejected       ) << " " << hitflags << std::endl;
	  ...
	  if (applySkipClean && skipNegProp_ && fwdUpdTkState.size()>0 && dist<negDistTolerance_) {
	    rejectedhsidx.push_back(ihit);
	    continue;
	  }
	  if (dist<min_dist) {
	...
	if (pickBestHitOnWire_) {
	  ...
	  for (unsigned int jh = iterHitsInPlanes[min_plane]; jh<hitsInPlanes[min_plane].size(); ++jh) {
	    ...
	    if (dumpLevel_>1 && ihit != jhit) std::cout << "additional hit on the same wire with jhit=" << jhit << " chi2=" << chi2 << " ihit=" << ihit << " min_chi2_wire=" << min_chi2_wire << std::endl;
	    if (chi2<min_chi2_wire) {
	      min_chi2_wire=chi2;
	      if (dumpLevel_>1 && ihit != jhit) {std::cout << "\tnow using "; jhitstate.dump();}
  ...
  if (dumpLevel_>0) {
    ...
    std::cout << "hit sizes=" << rejectedhsidx.size() << " " << hitstateidx.size() << " " <<  hitstatev.size() << std::endl;
    trackState.dump();
  }
...
bool trkf::TrackKalmanFitter::fillResult(const std::vector<art::Ptr<recob::Hit> >& inHits, const int tkID, const int pdgid,
  ...
  if (dumpLevel_>0) {
    std::cout << "outTrack vertex=" << outTrack.Start()
	      ...
	      << "\nchi2/ndof=" << outTrack.Chi2PerNdof()
	      << std::endl;
  }
