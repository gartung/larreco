...
public:
  ...
  void AssociateClustersAndTracks(
  ...
  /// Map associated tracks and clusters together given their associated hits,
  /// whilst ignoring certain clusters
  void AssociateClustersAndTracks(
    std::vector<art::Ptr<recob::Cluster>> const& clusters,
    art::FindManyP<recob::Hit> const& fmh,
    art::FindManyP<recob::Track> const& fmt,
  ...
  /// Takes the initial showers found and tries to resolve issues where one bad
  /// view ruins the event
  std::vector<int> CheckShowerPlanes(
    std::vector<std::vector<int>> const& initialShowers,
    std::vector<art::Ptr<recob::Cluster>> const& clusters,
    art::FindManyP<recob::Hit> const& fmh) const;

  /// Constructs a recob::Track from sets of hits in two views. Intended to be
  /// used to construct the initial first part of a shower. All PMA methods
  /// taken from the pma tracking algorithm (R. Sulej and D. Stefan). This
  /// implementation also orients the track in the correct direction if a map of
  /// shower centres (units [cm]) in each view is provided.
  ...
  /// Constructs a recob::Track from sets of hits in two views. Intended to be
  /// used to construct the initial first part of a shower. All methods taken
  /// from the pma tracking algorithm (R. Sulej and D. Stefan).
  std::unique_ptr<recob::Track> ConstructTrack(
    std::vector<art::Ptr<recob::Hit>> const& track1,
  ...
  std::vector<recob::SpacePoint> MakeSpacePoints(
    std::map<int, std::vector<art::Ptr<recob::Hit>>> hits,
    std::vector<std::vector<art::Ptr<recob::Hit>>>& hitAssns) const;

  /// Takes the hits associated with a shower and orders them so they follow the
  /// direction of the shower
  // std::map<int,std::vector<art::Ptr<recob::Hit> > >
  // OrderShowerHits(art::PtrVector<recob::Hit> const& shower, int plane);
  std::map<int, std::vector<art::Ptr<recob::Hit>>> OrderShowerHits(
    const art::PtrVector<recob::Hit>& shower,
    int plane) const;
...
private:
  ...
  bool CheckShowerHits_(std::map<int, std::vector<art::Ptr<recob::Hit>>> const&
                          showerHitsMap) const;

  /// Constructs a 3D point (in [cm]) to represent the hits given in two views
  TVector3 Construct3DPoint_(art::Ptr<recob::Hit> const& hit1,
                             art::Ptr<recob::Hit> const& hit2) const;

  /// Finds dE/dx for the track given a set of hits
  double FinddEdx_(std::vector<art::Ptr<recob::Hit>> const& trackHits,
  ...
  std::vector<art::Ptr<recob::Hit>> FindOrderOfHits_(
  ...
  TVector2 HitPosition_(recob::Hit const& hit) const;

  /// Return the coordinates of this hit in units of cm
  TVector2 HitPosition_(TVector2 const& pos, geo::PlaneID planeID) const;
  ...
  /// direction of the shower
  void OrderShowerHits_(std::vector<art::Ptr<recob::Hit>> const& shower,
  ...
  TVector2 Project3DPointOntoPlane_(TVector3 const& point,
                                    int plane,
...
public:
  ...
  TVector2
  ConvertWireTickToCm(TVector2 wiretick, geo::PlaneID planeID)
  {
