  ...
  TH1F* histoRejected) const
{
  ...
  for (auto c : trkCryos)
    for (auto t : trkTPCs) {
      ranges[std::pair<unsigned int, unsigned int>(t, c)] =
        trk.WireDriftRange(testPlane, t, c);
  ...
  auto hits_on_test_plane = [testPlane](auto const& hit) {
    return hit.WireID().Plane == testPlane;
  };
  for (auto const& h :
       hits | view::transform(to_element) | view::filter(hits_on_test_plane)) {
    tpc = h.WireID().TPC;
  ...
  for (auto const* seg : trk.Segments()) {
    ...
    auto const& points =
      all_close_points[std::pair<unsigned int, unsigned int>(tpc, cryo)];
...
bool
pma::ProjectionMatchingAlg::GetCloseHits(
  double r2d,
  const std::vector<art::Ptr<recob::Hit>>& hits_in,
  std::vector<size_t>& used,
...
void
pma::ProjectionMatchingAlg::ShortenSeg(pma::Track3D& trk,
...
bool
pma::ProjectionMatchingAlg::TestTrk(pma::Track3D& trk,
                                    const geo::TPCGeo& tpcgeom) const
{
  bool test = false;
...
bool
pma::ProjectionMatchingAlg::chkEndpointHits(
  int wire,
  int wdir,
  double drift_x,
  int view,
  ...
  unsigned int cryo) const
{
  ...
  for (unsigned int i = 0; i < 3; i++)
    if (wires[i].first >= 0) {
      ...
      if (hiter != hits.end()) {
        if (chkEndpointHits(wires[i].first,
                            wires[i].second,
                            xPos[i],
                            i,
                            tpc,
...
pma::ProjectionMatchingAlg::trimTrackToVolume(pma::Track3D& trk,
                                              TVector3 p0,
                                              TVector3 p1) const
{
                                              ...
                                              unsigned int* nused) const
{
  ...
  // std::cout << "nhits=" << nhits << ", dq=" << dq << ", dx=" << dx <<
  // std::endl;
  return dqdx;
}
// ------------------------------------------------------
