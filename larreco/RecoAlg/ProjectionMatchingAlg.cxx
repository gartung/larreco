...
// Author:      D.Stefan (Dorota.Stefan@ncbj.gov.pl) and R.Sulej
// (Robert.Sulej@cern.ch), May 2015
////////////////////////////////////////////////////////////////////////////////////////////////////
...
pma::ProjectionMatchingAlg::ProjectionMatchingAlg(
  const pma::ProjectionMatchingAlg::Config& config)
...
double
pma::ProjectionMatchingAlg::validate_on_adc(
  ...
  float const thr) const
{
  ...
  pma::Vector3D p(trk.front()->Point3D().X(),
                  trk.front()->Point3D().Y(),
                  trk.front()->Point3D().Z());
  for (auto const* seg : trk.Segments()) {
    if (seg->TPC() <
        0) // skip segments between tpc's, look only at those contained in tpc
    {
      p = seg->End();
      continue;
    }
    ...
    while ((f < 1.0) && node->SameTPC(p)) {
      ...
      if (fGeom->HasWire(wireID)) {
        ...
        if (channelStatus.IsGood(ch)) {
          float max_adc = adcImage.poolMax(
            widx, didx, 2); // +/- 2 wires, can be parameterized
          if (max_adc > thr)
            nPassed++;
...
double
pma::ProjectionMatchingAlg::validate_on_adc_test(
  const lariov::ChannelStatusProvider& channelStatus,
  const pma::Track3D& trk,
  const img::DataProviderAlg& adcImage,
  ...
  TH1F* histoRejected) const
{
  ...
  std::vector<unsigned int> trkTPCs = trk.TPCs();
  std::vector<unsigned int> trkCryos = trk.Cryos();
  std::map<std::pair<unsigned int, unsigned int>, std::pair<TVector2, TVector2>>
    ranges;
  std::map<std::pair<unsigned int, unsigned int>, double> wirePitch;
  for (auto const& [t, c] : view::cartesian_product(trkTPCs, trkCryos)) {
    ranges[{t, c}] = trk.WireDriftRange(testPlane, t, c);
    wirePitch[{t, c}] = fGeom->TPC(t, c).Plane(testPlane).WirePitch();
  }

  unsigned int tpc, cryo;
  std::map<std::pair<unsigned int, unsigned int>, std::vector<pma::Vector2D>>
    all_close_points;

  for (auto const& h : hits | view::transform(to_element) |
                         view::filter(hits_on_plane{testPlane})) {
    tpc = h.WireID().TPC;
    ...
    if ((h.WireID().Wire >
         rect.first.X() - 10) && // chceck only hits in the rectangle around
        (h.WireID().Wire <
         rect.second.X() + 10) && // the track projection, it is faster than
        (h.PeakTime() >
         rect.first.Y() - 100) && // calculation of trk.Dist2(p2d, testPlane)
        (h.PeakTime() < rect.second.Y() + 100)) {
      TVector2 p2d(
        wirePitch[tpc_cryo] * h.WireID().Wire,
        fDetProp->ConvertTicksToX(h.PeakTime(), testPlane, tpc, cryo));

      d2 = trk.Dist2(p2d, testPlane, tpc, cryo);

      if (d2 < max_d2) {
        all_close_points[tpc_cryo].emplace_back(p2d.X(), p2d.Y());
      }
    }
  }
    ...
    while ((f < 1.0) && node->SameTPC(p)) {
      ...
      if (fGeom->HasWire(wireID)) {
        ...
        if (channelStatus.IsGood(ch)) {
          ...
          if (is_close) {
            if (histoPassing)
              histoPassing->Fill(max_adc);
          } else {
            if (histoRejected)
              histoRejected->Fill(max_adc);
          }
        }
        // else mf::LogVerbatim("ProjectionMatchingAlg")
        //	<< "crossing BAD CHANNEL (wire #" << (int)p2d.X() << ")" <<
        // std::endl;
      }
...
double
pma::ProjectionMatchingAlg::validate(
  const lariov::ChannelStatusProvider& channelStatus,
  const pma::Track3D& trk,
  ...
  unsigned int cryo) const
{
  ...
  double wirepitch = fGeom->TPC(tpc, cryo).Plane(testPlane).WirePitch();
  while (f < 1.0) {
    TVector2 p2d(fGeom->WireCoordinate(p.Y(), p.Z(), testPlane, tpc, cryo),
                 p.X());
    geo::WireID wireID(cryo, tpc, testPlane, (int)p2d.X());
    if (fGeom->HasWire(wireID)) {
      raw::ChannelID_t ch = fGeom->PlaneWireToChannel(wireID);
        ...
        for (const auto& h : hits)
          if ((h->WireID().Plane == testPlane) && (h->WireID().TPC == tpc) &&
            ...
            d2 = pma::Dist2(
              p2d,
              pma::WireDriftToCm(
                h->WireID().Wire, h->PeakTime(), testPlane, tpc, cryo));
            if (d2 < max_d2) {
              nPassed++;
              break;
            }
  ...
  if (nAll > 3) // validate actually only if 2D projection in testPlane has some
                // minimum length
  {
    double v = nPassed / (double)nAll;
...
size_t
pma::ProjectionMatchingAlg::getSegCount_(size_t trk_size)
{
  ...
  const std::vector<art::Ptr<recob::Hit>>& hits_2) const
{
  ...
  if (!trk->Initialize()) {
    mf::LogWarning("ProjectionMatchingAlg")
  ...
  if (f > fMinTwoViewFraction) {
    double g = 0.0;
    mf::LogVerbatim("ProjectionMatchingAlg")
      << "  optimize trk (" << nSegments << " seg)";
    if (nNodes) {
      g = trk->Optimize(
        nNodes, fOptimizationEps, false, true, 25, 10); // build nodes
...
pma::Track3D*
pma::ProjectionMatchingAlg::buildMultiTPCTrack(
  const std::vector<art::Ptr<recob::Hit>>& hits) const
{
  ...
  if (!tracks.empty()) {
    ...
    if (nSegments > 0) // need to add segments
    {
      ...
      if (nNodes) {
        mf::LogVerbatim("ProjectionMatchingAlg")
          << "  optimize trk (add " << nSegments << " seg)";
...
pma::Track3D*
pma::ProjectionMatchingAlg::buildShowerSeg(
  const std::vector<art::Ptr<recob::Hit>>& hits,
  const pma::Vector3D& vtx) const
{
  double vtxarray[3]{vtx.X(), vtx.Y(), vtx.Z()};

  if (!fGeom->HasTPC(fGeom->FindTPCAtPosition(vtxarray)))
    return 0;
  ...
  size_t countviews = 0;
  while (view < 3) {
    mf::LogVerbatim("ProjectionMatchinAlg")
...
void
pma::ProjectionMatchingAlg::FilterOutSmallParts(
  double r2d,
  ...
  const TVector2& vtx2d) const
{
  ...
  size_t id = 0;
  while (GetCloseHits_(r2d, hits_in, used, close_hits)) {
  ...
  for (size_t i = 0; i < sub_groups.size(); ++i) {
    if ((i != id_sub_groups_save) && (hits_out.size() < 10) &&
        (sub_groups[i].size() > min_size))
      for (auto h : sub_groups[i])
        hits_out.push_back(h);
  }
}
...
bool
pma::ProjectionMatchingAlg::GetCloseHits_(
  double r2d,
  const std::vector<art::Ptr<recob::Hit>>& hits_in,
  std::vector<size_t>& used,
    ...
    while (collect) {
      ...
      for (size_t i = 0; i < hits_in.size(); i++)
        if (!Has_(used, i)) {
          ...
          TVector2 hi_cm(wirePitch * hi->WireID().Wire,
                         driftPitch * hi->PeakTime());

          bool accept = false;

          for (auto const& ho : hits_out) {

            TVector2 ho_cm(wirePitch * ho->WireID().Wire,
                           driftPitch * ho->PeakTime());
...
void
pma::ProjectionMatchingAlg::ShortenSeg_(pma::Track3D& trk,
...
bool
pma::ProjectionMatchingAlg::TestTrk_(pma::Track3D& trk,
                                     const geo::TPCGeo& tpcgeom) const
{
  bool test = false;
...
pma::Track3D*
pma::ProjectionMatchingAlg::buildSegment(
  const std::vector<art::Ptr<recob::Hit>>& hits_1,
  const std::vector<art::Ptr<recob::Hit>>& hits_2) const
{
  pma::Track3D* trk = new pma::Track3D();
...
bool
pma::ProjectionMatchingAlg::chkEndpointHits_(
  int wire,
  int wdir,
  double drift_x,
  int view,
  ...
  unsigned int cryo) const
{
  ...
  for (unsigned int i = 0; i < 3; i++)
    if (wires[i].first >= 0) {
      ...
      if (hiter != hits.end()) {
        if (chkEndpointHits_(wires[i].first,
                             wires[i].second,
                             xPos[i],
                             i,
                             tpc,
  ...
  if (wire_view.size() > 1) {
    ...
    mf::LogVerbatim("ProjectionMatchingAlg")
      << "trk tpc:" << tpc << " size:" << trk.size() << " add ref.point (" << x
      << "; " << y << "; " << z << ")";
    return true;
  } else {
    mf::LogVerbatim("ProjectionMatchingAlg")
  ...
  const std::map<unsigned int, std::vector<art::Ptr<recob::Hit>>>& hits) const
{
  ...
  std::pair<int, int> wiresFront[3],
    wiresBack[3]; // wire index; index direction
    ...
    if (fGeom->TPC(tpc, cryo).HasPlane(i)) {
      ...
      if ((idxFront0 >= 0) && (idxFront0 < (int)trk.size()) &&
        ...
        if ((idxFront1 >= 0) && (idxFront1 < (int)trk.size()) &&
          ...
          xFront[i] =
            fDetProp->ConvertTicksToX(trk[idxFront0]->PeakTime(), i, tpc, cryo);

          wiresBack[i].first = wBack0;
          wiresBack[i].second = wBack0 - wBack1;
          xBack[i] =
            fDetProp->ConvertTicksToX(trk[idxBack0]->PeakTime(), i, tpc, cryo);
  ...
  if ((nPlanesFront > 1) && (fabs(dirFrontXZ.Z()) >= maxCosXZ)) {
    refAdded |= addEndpointRef_(trk, hits, wiresFront, xFront, tpc, cryo);
  }

  if ((nPlanesBack > 1) && (fabs(dirBackXZ.Z()) >= maxCosXZ)) {
    refAdded |= addEndpointRef_(trk, hits, wiresBack, xBack, tpc, cryo);
  }
  if (refAdded) {
    mf::LogVerbatim("ProjectionMatchingAlg")
  ...
  const std::map<unsigned int, std::vector<art::Ptr<recob::Hit>>>& hits) const
{
  ...
  if (fabs(dir0XZ.Z()) < maxCosXZ) {
    return;
  } // not parallel to wire planes => exit
    ...
    if (fGeom->TPC(tpc, cryo).HasPlane(i)) {
      ...
      if ((idx0 >= 0) && (idx0 < (int)trk.size()) &&
        ...
        if ((idx1 >= 0) && (idx1 < (int)trk.size()) &&
          ...
          x0[i] =
            fDetProp->ConvertTicksToX(trk[idx0]->PeakTime(), i, tpc, cryo);
  ...
  if ((nPlanes > 1) && (fabs(dir0XZ.Z()) >= maxCosXZ) &&
      addEndpointRef_(trk, hits, wires, x0, tpc, cryo)) {
    mf::LogVerbatim("ProjectionMatchingAlg")
...
pma::ProjectionMatchingAlg::trimTrackToVolume(pma::Track3D&,
                                              TVector3,
                                              TVector3) const
{
  return {};
...
bool
pma::ProjectionMatchingAlg::alignTracks(pma::Track3D& first,
                                        pma::Track3D& second) const
{
  unsigned int k = 0;
  double const distFF =
    pma::Dist2(first.front()->Point3D(), second.front()->Point3D());
  double dist = distFF;

  double const distFB =
    pma::Dist2(first.front()->Point3D(), second.back()->Point3D());
  if (distFB < dist) {
    k = 1;
    dist = distFB;
  }
  ...
  double const distBF =
    pma::Dist2(first.back()->Point3D(), second.front()->Point3D());
  if (distBF < dist) {
    k = 2;
    dist = distBF;
  }
  ...
  switch (k) // flip to get dst end before src start, do not merge if track's
             // order reversed
  {
    case 0:
      first.Flip();
      break;
    case 1:
      mf::LogError("PMAlgTrackMaker") << "Tracks in reversed order.";
      return false;
    case 2:
      break;
    case 3:
                                              ...
                                              unsigned int* nused) const
{
  for (size_t i = 0; i < trk.size(); i++) {
    pma::Hit3D* hit = trk[i];
    if (hit->View2D() == view) {
      if ((hit->GetDistToProj() >
           0.5) || // more than 0.5cm away away from the segment
          (hit->GetSegFraction() <
           -1.0)) // projects before segment start (to check!!!)
        hit->TagOutlier(true);
      else
  ...
  if (!nhits) {
    mf::LogError("ProjectionMatchingAlg")
