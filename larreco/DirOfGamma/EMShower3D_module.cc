...
ems::EMShower3D::ConvertFrom(pma::Track3D& src)
{
  ...
  double avdrift =
    (src.front()->Point3D().X() + src.back()->Point3D().X()) * 0.5;
  ...
  for (unsigned int p = 0; p < nplanes; ++p) {
    ...
    double dqdxplane =
      fProjectionMatchingAlg.selectInitialHits(src, p, &nusedP);
    double timeP = detprop->ConvertXToTicks(
      avdrift, p, src.front()->TPC(), src.front()->Cryo());
...
ems::EMShower3D::produce(art::Event& e)
{
  ...
  std::unique_ptr<std::vector<recob::Track>> tracks(
    new std::vector<recob::Track>);
  std::unique_ptr<std::vector<recob::Vertex>> vertices(
    new std::vector<recob::Vertex>);
  std::unique_ptr<std::vector<recob::Cluster>> clusters(
    new std::vector<recob::Cluster>);
  std::unique_ptr<std::vector<recob::SpacePoint>> allsp(
  ...
  if (e.getByLabel(fCluModuleLabel, fCluListHandle)) {
    ...
    // create cluster from hits, which were an input to find initial part of the
    // cascade.
    fClIndex = 0;
...
ems::EMShower3D::Link(art::Event const& e, std::vector<ems::DirOfGamma*> input)
{
  ...
  while (i < input.size()) {
    ...
    float t1 = detprop->ConvertTicksToX(
      input[i]->GetFirstHit()->PeakTime(), startview, tpc, cryo);
    ...
    for (unsigned int j = 0; j < input.size(); j++) {
      ...
      if ((i != j) && (secondview != startview) && (tpc == tpc_j) &&
          (cryo == cryo_j)) {
        float t2 = detprop->ConvertTicksToX(
          input[j]->GetFirstHit()->PeakTime(), secondview, tpc_j, cryo_j);
                                ...
                                size_t id)
{
  ...
  if (input[id]->GetCandidates().size() < 2) {
    return index;
  }
                           ...
                           std::vector<ems::DirOfGamma*> pair)
{
  ...
  if (tpc1 == tpc2)
    for (size_t i = 0; i < vec1.size(); ++i)
      for (size_t j = 0; j < vec2.size(); ++j)
        if ((vec1[i]->WireID().TPC == vec2[j]->WireID().TPC) &&
            (tpc1 == vec2[j]->WireID().TPC)) {
  ...
  if ((hitscl1uniquetpc.size() > 2) && (hitscl2uniquetpc.size() > 2)) {
    pma::Track3D* trk =
      fProjectionMatchingAlg.buildSegment(hitscl1uniquetpc, hitscl2uniquetpc);
                          ...
                          size_t plane3)
{
  ...
  std::vector<art::Ptr<recob::Hit>> vec1 =
    input[id1]->GetCandidates()[c1].GetIniHits();
  std::vector<art::Ptr<recob::Hit>> vec2 =
    input[id2]->GetCandidates()[c2].GetIniHits();
  ...
  if ((hitscl1uniquetpc.size() < 3) || (hitscl2uniquetpc.size() < 3))
    return false;
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    for (size_t h = 0; h < hits2dcl.size(); ++h) {
      TVector2 pfront = pma::GetProjectionToPlane(track->front()->Point3D(),
                                                  plane3,
                                                  track->FrontTPC(),
                                                  track->FrontCryo());
...
bool
ems::EMShower3D::Validate(art::Event const& e,
...
void
ems::EMShower3D::FilterOutSmallParts(
  double r2d,
  const std::vector<art::Ptr<recob::Hit>>& hits_in,
  std::vector<art::Ptr<recob::Hit>>& hits_out)
{
