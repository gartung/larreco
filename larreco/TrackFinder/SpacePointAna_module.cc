...
namespace trkf {
    ...
    class SpacePointAna : public art::EDAnalyzer
    {
        ...
        private:
            ...
            // Tree
            TTree *t;
            std::vector<double> h_dt_UV;
            std::vector<double> h_dt_VW;
            std::vector<double> h_dt_WU;
            std::vector<double> h_posX;
            std::vector<double> h_posY;
            std::vector<double> h_posZ;
            std::vector<double> h_trackTheta;
            std::vector<double> h_trackPhi;
            std::vector<double> h_trackLength;
    };
          ...
          , fNumEvent(0)
    {
        ...
        mf::LogInfo("SpacePointAna") 
            ...
            << "  SpacePointModuleLabel = " << fSpacePointModuleLabel << "\n"
            << "  PfpModuleLabel = " << fPfpModuleLabel << "\n"
        ...
        //
    {
        if(!fBooked) {
            ...
            t = dir.make<TTree>("spacepoint_tree", "spacepoint_tree");
            initTree(t);
        ...
        //
    {
        ...
        art::FindManyP<recob::Track> fm_tracks(hitHandle, evt, fTrackModuleLabel);

        // Loop over tracks and get angles
        for (size_t i = 0; i < trackPtrVector.size(); i++) {
            art::Ptr<recob::Track> thisTrack = trackPtrVector.at(i);
            h_trackTheta.push_back( thisTrack->Theta() );
            h_trackPhi.push_back( thisTrack->Phi() );
            h_trackLength.push_back( thisTrack->Length() );
        }
                    ...
                    for(art::PtrVector<recob::Hit>::const_iterator jhit = spthits.begin();
                        ...
                        if(tpc1 == tpc2 && plane1 != plane2) {
                            ...
                            if(view1 == geo::kU) {
                                if(view2 == geo::kV) {
                                    fHDTUV->Fill(t1-t2);
                                    h_dt_UV.push_back(t1-t2);
                                }
                                if(view2 == geo::kZ) {
                                    fHDTWU->Fill(t2-t1);
                                    h_dt_WU.push_back(t2-t1);
                                }
                            }
                            if(view1 == geo::kV) {
                                if(view2 == geo::kZ) {
                                    fHDTVW->Fill(t1-t2);
                                    h_dt_VW.push_back(t1-t2);
                                }
                                if(view2 == geo::kU) {
                                    fHDTUV->Fill(t2-t1);
                                    h_dt_UV.push_back(t2-t1);
                                }
                            }
                            if(view1 == geo::kZ) {
                                if(view2 == geo::kU) {
                                    fHDTWU->Fill(t1-t2);
                                    h_dt_WU.push_back(t1-t2);
                                }
                                if(view2 == geo::kV) {
                                    fHDTVW->Fill(t2-t1);
                                    h_dt_VW.push_back(t2-t1);
                                }
                            }
                        }
                    }
        ...
        for(std::vector<recob::SpacePoint>::const_iterator i = spts3.begin();
            ...
            h_posX.push_back(spt.XYZ()[0]);
            h_posY.push_back(spt.XYZ()[1]);
            h_posZ.push_back(spt.XYZ()[2]);
    ...
    void SpacePointAna::initTree(TTree *t) {
        t->Branch("run", &run);
        t->Branch("subrun", &subrun);
        t->Branch("event", &event);
        t->Branch("ntracks", &ntracks);
        t->Branch("h_dt_UV", &h_dt_UV);
        t->Branch("h_dt_VW", &h_dt_VW);
        t->Branch("h_dt_WU", &h_dt_WU);
        t->Branch("h_posX", &h_posX);
        t->Branch("h_posY", &h_posY);
        t->Branch("h_posZ", &h_posZ);
        t->Branch("h_trackTheta", &h_trackTheta);
        t->Branch("h_trackPhi", &h_trackPhi);
        t->Branch("h_trackLength", &h_trackLength);
    }
