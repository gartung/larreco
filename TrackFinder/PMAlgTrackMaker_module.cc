...
typedef std::vector<TrkCandidate>          TrkCandidateVec;
typedef std::map< size_t, TrkCandidateVec> TPCTrackCandMap;
...
private:
    ...
    int fromMaxCluster(const art::Event& evt, TrkCandidateVec& result);
    
    void fromMaxCluster_tpc(
//                            std::vector< pma::Track3D* >& result,
                            TrkCandidateVec& result,
                            const std::vector< art::Ptr< recob::Cluster >>& clusters,
                            const art::FindManyP< recob::Hit >& fbp,
                            size_t minBuildSize, unsigned int tpc, unsigned int cryo, int pfParticleIdx=-1);
    ...
    int fromExistingAssocs(const art::Event& evt, TrkCandidateVec& result);
    // ------------------------------------------------------
    ...
    bool areCoLinear(
                     ...
                     double distProjThr,
                     double cosThr);
    void mergeCoLinear(TrkCandidateVec& tracks);
    void mergeCoLinear(TPCTrackCandMap& tracks);
    
    bool reassignHits(
                      const std::vector< art::Ptr<recob::Hit> >& hits,
                      TrkCandidateVec& tracks, size_t trk_idx);
    bool reassignSingleViewEnds(TrkCandidateVec& tracks);
    
    double validate(pma::Track3D& trk, unsigned int testView);
    double validate(pma::Track3D& trk, const std::vector<art::Ptr<recob::Hit>>& hitVec, unsigned int testView);
...
double PMAlgTrackMaker::validate(pma::Track3D& trk, const std::vector<art::Ptr<recob::Hit>>& hitVec, unsigned int testView)
{
    if (testView != geo::kUnknown)
        mf::LogVerbatim("PMAlgTrackMaker") << "validation in plane: " << testView;
    else return 1.0;
    
    // Override the c_t_v_hits structure with current hits
    size_t cryo = trk.FrontCryo();
    size_t tpc  = trk.FrontTPC();
    
    c_t_v_hits[cryo][tpc].clear();
    
    for(const auto& hit : hitVec)
    {
        const geo::WireID& wire = hit->WireID();
        
        if (wire.Plane != testView || wire.Cryostat != cryo || wire.TPC != tpc) continue;

        c_t_v_hits[cryo][tpc][testView].push_back(hit);
    }
...
void PMAlgTrackMaker::mergeCoLinear(TrkCandidateVec& tracks)
{
...
void PMAlgTrackMaker::mergeCoLinear(TPCTrackCandMap& tracks)
{
    ...
    for (auto & tpc_entry1 : tracks)
    {
        ...
        TrkCandidateVec& tracks1 = tpc_entry1.second;
        ...
        while (t < tracks1.size())
        {
            ...
            if ((dfront1 < wallDistThr) || (dback1 < wallDistThr))
            {
                for (auto & tpc_entry2 : tracks)
                {
                    ...
                    TrkCandidateVec& tracks2 = tpc_entry2.second;
...
bool PMAlgTrackMaker::reassignSingleViewEnds(TrkCandidateVec& tracks)
{
    bool result = false;
...
void PMAlgTrackMaker::produce(art::Event& evt)
{
    ...
    // Fill hits by Cryostat, TPC and view
    if (fCluMatchingAlg == 1)
    {
        if (!sortHits(evt))
        {
    ...
    if (result.size()) // ok, there is something to save
    {
        ...
        fTrkIndex = 0;
        for (auto const& trkCand : result)
        {
            if (fFlipToBeam)    // flip the track to the beam direction
            {
                double z0 = trkCand.Track->front()->Point3D().Z();
                double z1 = trkCand.Track->back()->Point3D().Z();
                if (z0 > z1) trkCand.Track->Flip();
            }
            if (fFlipDownward)  // flip the track to point downward
            {
                double y0 = trkCand.Track->front()->Point3D().Y();
                double y1 = trkCand.Track->back()->Point3D().Y();
                if (y0 < y1) trkCand.Track->Flip();
            }
            if (fAutoFlip_dQdx) // flip the track by dQ/dx
                fProjectionMatchingAlg.autoFlip(*trkCand.Track, pma::Track3D::kForward, dQdxFlipThr);
            /* test code: fProjectionMatchingAlg.autoFlip(*trk, pma::Track3D::kBackward, dQdxFlipThr); */
            
            tracks->push_back(convertFrom(*trkCand.Track));
            fTrkIndex++;
            ...
            spStart = allsp->size();
            for (int h = trkCand.Track->size() - 1; h >= 0; h--)
            {
                pma::Hit3D* h3d = (*trkCand.Track)[h];
            ...
            if (trkCand.pfParticleIdx > -1)
            {
                ...
                pfPartToTrackVecMap[trkCand.pfParticleIdx].push_back(trackPtr);
            }
        }
        ...
        for (size_t t = 0; t < result.size(); t++) delete result[t].Track;
    }
...
int PMAlgTrackMaker::fromMaxCluster(const art::Event& evt, TrkCandidateVec& result)
{
    ...
    if (evt.getByLabel(fCluModuleLabel, cluListHandle))
    {
        ...
        art::fill_ptr_vector(clusterVec, cluListHandle);
        ...
        for (auto tpc_iter = fGeom->begin_TPC_id();
             ...
             tpc_iter++)
        {
            fromMaxCluster_tpc(tpcTrackMap[tpc_iter->TPC], clusterVec, fbp, minBuildSizeLarge, tpc_iter->TPC, tpc_iter->Cryostat);
        }
        ...
        for (auto tpc_iter = fGeom->begin_TPC_id();
             ...
             tpc_iter++)
        {
            fromMaxCluster_tpc(tpcTrackMap[tpc_iter->TPC], clusterVec, fbp, minBuildSizeSmall, tpc_iter->TPC, tpc_iter->Cryostat);
        }
        ...
        for (auto tpc_iter = fGeom->begin_TPC_id();
             ...
             tpc_iter++)
        {
            reassignSingleViewEnds(tpcTrackMap[tpc_iter->TPC]);
        }
        ...
        if (fMergeWithinTPC)
        {
                 ...
                 tpc_iter++)
            {
                ...
                mergeCoLinear(tpcTrackMap[tpc_iter->TPC]);
            }
        }
        ...
        if (fStitchBetweenTPCs)
        {
            ...
            mergeCoLinear(tpcTrackMap);
        }
        
        for (auto const & tpc_entry : tpcTrackMap)
            for (auto const & trkCand : tpc_entry.second)
                result.push_back(trkCand);
                                  ...
                                  unsigned int tpc, unsigned int cryo)
{
    ...
    for (size_t i = 0; i < clusters.size(); ++i)
    {
        ...
        std::vector< art::Ptr<recob::Hit> > v = fbp.at(clusters[i].key());
...
void PMAlgTrackMaker::fromMaxCluster_tpc(
                                         ...
                                         const std::vector< art::Ptr< recob::Cluster >>& clusters,
                                         const art::FindManyP< recob::Hit >& fbp,
                                         size_t minBuildSize, unsigned int tpc, unsigned int cryo, int pfParticleIdx)
{
    initial_clusters.clear();
    ...
    while (max_first_idx >= 0) // loop over clusters, any view, starting from the largest
    {
        ...
        if (max_first_idx >= 0)
        {
            const art::Ptr<recob::Cluster>& clu_first = clusters[max_first_idx];
            ...
            std::vector< art::Ptr<recob::Hit> > v_first = fbp.at(clu_first.key());
            ...
            while (try_build) // loop over complementary views
            {
                ...
                candidate.pfParticleIdx = pfParticleIdx;
                ...
                if (try_build)
                {
                    ...
                    candidate.Track = fProjectionMatchingAlg.buildTrack(v_first, fbp.at(clusters[idx].key()));
                    
                    if (candidate.Track) // no track if hits from 2 views do not alternate
                    {
                        m0 = candidate.Track->GetMse();
                        if (m0 < 0.15) v0 = validate(*(candidate.Track), fbp.at(clusters[idx].key()), testView);
                    }
                    if (candidate.Track && (m0 < 0.15) && (v0 > 0.7)) // good candidate, try to extend it
                    {
                        ...
                        while (idx >= 0) // try to collect matching clusters, use **any** plane except validation
                        {
                            ...
                            if (idx >= 0)
                            {
                                ...
                                if (extendTrack(candidate, fbp.at(clusters[idx].key()),  testView,    true))
                                {
                                    candidate.Clusters.push_back(clusters[idx].key());
                                }
                                else idx = -1;
                            }
                        }
                        ...
                        {	//                     match clusters from the plane used previously for the validation
                            ...
                            if (idx >= 0)
                            {
                                ...
                                if (extendTrack(candidate, fbp.at(clusters[idx].key()), geo::kUnknown, false))
                                {
                                    candidate.Clusters.push_back(idx);
                                }
                                else idx = -1;
            ...
            if (!fCandidates.empty()) // save best candidate, release other tracks and clusters
            {
                // Let's make a slight change to the below to make sure we don't simply pick up the first track all the time
                int    best_trk = -1;
                ...
                // Now handle deleting unused tracks
                for (size_t t = 0; t < fCandidates.size(); t++)
                {
                    if (int(t) != best_trk) delete fCandidates[t].Track;   // <<< there is meant to be a "delete" here, inserting now
                }
                
                // We should simply clear here?
                fCandidates.clear();
            }
        }
...
int PMAlgTrackMaker::fromExistingAssocs(const art::Event& evt, TrkCandidateVec& result)
{
    ...
    if (evt.getByLabel(fCluModuleLabel, pfParticleHandle))
    {
        ...
        if (clusterHandle.isValid())
        {
            ...
            art::FindManyP<recob::Hit> clusterHitAssns(clusterHandle, evt, fCluModuleLabel);
            
            //tpc_track_map tracks; // track parts in tpc's
            TPCTrackCandMap trackCandMap;
            ...
            for(size_t pfPartIdx = 0; pfPartIdx < pfParticleHandle->size(); pfPartIdx++)
            {
                ...
                for (auto tpc_iter = fGeom->begin_TPC_id();
                     ...
                     tpc_iter++)
                {
                    fromMaxCluster_tpc(trackCandMap[tpc_iter->TPC], clusterVec, clusterHitAssns, minBuildSizeLarge, tpc_iter->TPC, tpc_iter->Cryostat, pfParticle.key());
                }
                ...
                for (auto tpc_iter = fGeom->begin_TPC_id();
                     ...
                     tpc_iter++)
                {
                    fromMaxCluster_tpc(trackCandMap[tpc_iter->TPC], clusterVec, clusterHitAssns, minBuildSizeSmall, tpc_iter->TPC, tpc_iter->Cryostat, pfParticle.key());
                }
            ...
            for (auto tpc_iter = fGeom->begin_TPC_id();
                 ...
                 tpc_iter++)
            {
                reassignSingleViewEnds(trackCandMap[tpc_iter->TPC]);
            }
            ...
            if (fMergeWithinTPC)
            {
                ...
                {
                    mf::LogVerbatim("PMAlgTrackMaker") << "Merge co-linear tracks within TPC " << tpc_iter->TPC << ".";
                    mergeCoLinear(trackCandMap[tpc_iter->TPC]);
                }
            }
            ...
            {
                mf::LogVerbatim("PMAlgTrackMaker") << "Stitch co-linear tracks between TPCs.";
                mergeCoLinear(trackCandMap);
            }
            
            for (auto const & tpc_entry : trackCandMap)
                for (auto const & trkCand : tpc_entry.second)
                    result.push_back(trkCand);
        }
    }
