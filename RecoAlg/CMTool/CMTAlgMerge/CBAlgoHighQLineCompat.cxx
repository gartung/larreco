...
namespace cmtool {
  ...
  const std::vector<Hit2D> CBAlgoHighQLineCompat::GetHighQHits(const std::vector<Hit2D> &pxhit_vector, double frac){
    ...
    std::vector<Hit2D> myvec = pxhit_vector;
    ...
    return std::vector<Hit2D>(myvec.begin(),myvec.begin()+nhits_reduced);

  }

  bool CBAlgoHighQLineCompat::SortPxHitsMethod(const Hit2D &h1, const Hit2D &h2){
