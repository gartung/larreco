...
namespace cmtool {
  ...
  */
  ...
  protected:
    ...
    double ComputeSlope(const std::vector<Hit2D> &pxhit_vector);
    ...
    const std::vector<Hit2D> GetHighQHits(const std::vector<Hit2D> &pxhit_vector, double frac);

    /// Function needed to sort pxhit vector by charge (static b/c that's the only way it'll compile...)
    static bool SortPxHitsMethod(const Hit2D &h1, const Hit2D &h2);
