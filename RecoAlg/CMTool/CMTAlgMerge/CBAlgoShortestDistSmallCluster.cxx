...
bool CBAlgoShortestDistSmallCluster::Bool(const ::cluster::cluster_params &cluster1,
    const ::cluster::cluster_params &cluster2)
{

  //if number of hits not large enough skip
  if ( (_minHits > 0) and ((cluster1.N_Hits < _minHits) or (cluster2.N_Hits < _minHits)) ) {
    return false;
  }

  //if number of hits is too large
  if ( (_maxHits > _minHits) and ((cluster1.N_Hits > _maxHits) or (cluster2.N_Hits > _maxHits)) ) {
    return false;
  }
  ...
  if (_debug) {
    std::cout << "Start point Cluster 1: (" << cluster1.start_point.w << ", " << cluster1.start_point.t << ")"  << std::endl;
    std::cout << "End point Cluster 2: (" << cluster1.end_point.w << ", " << cluster1.end_point.t << ")"  << std::endl;
    std::cout << "Start point Cluster 1: (" << cluster2.start_point.w << ", " << cluster2.start_point.t << ")"  << std::endl;
    std::cout << "End point Cluster 2: (" << cluster2.end_point.w << ", " << cluster2.end_point.t << ")"  << std::endl;
  }
...
double CBAlgoShortestDistSmallCluster::ShortestDistanceSquared(double point_x, double point_y,
