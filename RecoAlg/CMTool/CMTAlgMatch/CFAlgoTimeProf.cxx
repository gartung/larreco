...
//----------------------------------------------------------------------------------------------
{
  ...
  std::vector<Hit2D> hits0;
  std::vector<Hit2D> hits1;
  std::vector<Hit2D> hits2;
...
float CFAlgoTimeProf::TProfCompare(std::vector<Hit2D> hita , std::vector<Hit2D> hitb)
{
//   int nts = detProp->NumberTimeSamples()*util::GeometryHelper::GetME()->TimeToCm();
  // Where is this?
  ...
  double time_diff = ( detProp->GetXTicksOffset(hita.at(0).plane,0,0) -
                       detProp->GetXTicksOffset(hitb.at(0).plane,0,0) )
                     * util::GeometryHelper::GetME()->TimeToCm();

  // First go look for the min & max of hits
  double min_time = detProp->NumberTimeSamples()
                    * util::GeometryHelper::GetME()->TimeToCm();
  double max_time = 0;
  ...
  for ( auto const& ha : hita) {
    double time = ha.t;
    //time -= detProp->GetXTicksOffset(ha.plane)*util::GeometryHelper::GetME()->TimeToCm();
  ...
  for ( auto const& hb : hitb) {
    ...
    //time -= detProp->GetXTicksOffset(hb.plane)*util::GeometryHelper::GetME()->TimeToCm();
